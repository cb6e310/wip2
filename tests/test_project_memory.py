...
class ProjectMemoryTests(unittest.TestCase):
    ...
    def test_ready_named_by_blocker_fails(self) -> None:
        self.state["blockers"][0]["blocks"].append("S0_N2_SAMPLER")
    ...
    def test_v27_current_spec_passes(self) -> None:
        self.assertEqual(self._errors(), [])

    def test_v27_task_counts_and_ready_set(self) -> None:
        self.assertEqual(len(self.tasks), 74)
        self.assertEqual(
            sum(task["status"] == "DONE" for task in self.tasks.values()), 41
        )
        self.assertEqual(
    ...
    def test_v27_dependency_rewrite_is_exact(self) -> None:
    ...
    def test_v27_a_policy_tamper_fails(self) -> None:
        ...
        self._assert_code(CHECKER.validate(self.root), "V27_ARTIFACT_HASH_MISMATCH")

    def test_v27_frozen_split_tamper_fails(self) -> None:
    ...
    def test_v27_a_contract_tamper_fails(self) -> None:
        path = self.root / "artifacts/backbone_a_contract.yaml"
        path.write_bytes(path.read_bytes() + b"\n# tamper\n")
        self._assert_code(CHECKER.validate(self.root), "V27_ARTIFACT_HASH_MISMATCH")

    def test_v27_b9_is_closed_and_b4_does_not_block_resolver(self) -> None:
        ...
        self.assertNotIn("S0_N2_SAMPLER", b4["blocks"])
        self.assertIn("GATE_R0", b4["blocks"])

    def test_v27_feasibility_artifact_tamper_fails(self) -> None:
    ...
    def test_v27_ledger_tamper_fails(self) -> None:
        path = self.root / "artifacts/nulls/n1_block_assignment_v1.jsonl"
        path.write_bytes(path.read_bytes() + b"\n")
        self._assert_code(CHECKER.validate(self.root), "V27_ARTIFACT_HASH_MISMATCH")

    def test_v27_n1_contract_tamper_fails(self) -> None:
    ...
    def test_v27_manifest_tamper_fails(self) -> None:
        path = self.root / "artifacts/nulls/n1_permutation_manifest_v1.jsonl"
        path.write_bytes(path.read_bytes() + b"\n")
        self._assert_code(CHECKER.validate(self.root), "V27_ARTIFACT_HASH_MISMATCH")

    def test_v27_branch_owner_tamper_fails(self) -> None:
        self.tasks["S0_N2_SAMPLER"]["owner"] = "CODEX"
        self._assert_code(self._errors(), "V27_READY_SET_MISMATCH")

    def test_v27_branch_status_tamper_fails(self) -> None:
        self.tasks["S0_N2_SAMPLER"]["status"] = "BLOCKED"
        self.tasks["S0_N2_SAMPLER"]["blocked_reason"] = "fixture"
        self._assert_code(self._errors(), "V27_TASK_STATE_MISMATCH")

    def test_v27_frontend_validator_tamper_fails(self) -> None:
        path = self.root / "scripts/validate_a1_frontend.py"
        path.write_bytes(path.read_bytes() + b"\n# tamper\n")
        self._assert_code(CHECKER.validate(self.root), "V27_ARTIFACT_HASH_MISMATCH")

    def test_v27_leakage_artifact_tamper_fails(self) -> None:
        path = self.root / "artifacts/a_path_leakage_assertions.yaml"
        path.write_bytes(path.read_bytes() + b"\n# tamper\n")
        self._assert_code(CHECKER.validate(self.root), "V27_ARTIFACT_HASH_MISMATCH")

    def test_v27_admission_ledger_tamper_fails(self) -> None:
        path = self.root / "artifacts/a1_outer_train_admission_v1.jsonl"
        path.write_bytes(path.read_bytes() + b"\n")
        self._assert_code(CHECKER.validate(self.root), "V27_ARTIFACT_HASH_MISMATCH")

    def test_v27_admission_freeze_tamper_fails(self) -> None:
        ...
        self._assert_code(errors, "V27_ARTIFACT_HASH_MISMATCH")

    def test_v27_b9_active_again_fails(self) -> None:
        b9 = next(item for item in self.state["superseded_blockers"] if item["id"] == "B_V9_A_FULL_OUTER_TRAIN_ADMISSION_PENDING")
        ...
        self._assert_code(self._errors(), "V27_B9_CLOSURE_MISMATCH")
    ...
    def test_n2_sampler_requires_author_owner(self) -> None:
        self.tasks["S0_N2_SAMPLER"]["owner"] = "CODEX"
        self._assert_code(self._errors(), "V27_READY_SET_MISMATCH")
