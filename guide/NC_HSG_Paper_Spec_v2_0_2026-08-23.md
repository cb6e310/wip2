# NC-HSG 小论文统一规格（综合整合版 v2.0，2026-08-23）
...
本文件是 `NC_HSG_Paper_Spec.md` 与 `NC_HSG_Paper_Spec_v1.md` 的逐项审阅、冲突裁决和统一规格，并吸收实现前的算法、随机化检验、风险校准、仓库治理、输入准入、stimulus identity/grouping 与第九次 joint-split/population 审查。它是论文、实验和实现的共同合同；不是结果报告。文中所有【新】数值均必须在正式实验前冻结，所有【核】事实在核实前不得写成论文结论。v2.0 接受 run 010 的 342 个冻结刺激组，并在任何训练、calibration 或 outcome 读取前冻结 Regime I/II 的确定性切分、calibration reserve、test lock 与 subject-macro population contract；Codex 只负责实现和验证，不得挑 seed、搜索 split 或顺手选择 backbone。v2.0 不改变 v1.2–v1.9 的科学指标、Gate、grouping policy、数据分析视图、unknown-unit policy 或失败路线。§12–§18 是不可改写的历史快照，§19 是当前状态与下一动作的权威增量。

> **版本权威**：v2.0 是下一次 Codex 导入后应激活的 SPEC。导入前远程 `main@e852e7de24c31410387ad46d75fb44a5cac9e850` 已正确激活 v1.9，并完成 run 010 的冻结 stimulus identity/grouping：349 occurrences、344 exact identities、342 groups，kind counts 335/5/2，全部文本与 paraphrase 结论保持不可见/未验证。下一 run 实现 `S0_JOINT_SPLIT` 与紧邻的 `S0_GATE_A_POPULATION_E5`，成功后把 outcome-blind `S0_A_POLICY_REVIEW` 置为 READY 并停止；不得读取 test outcome、执行 leakage audit、选择 A、训练或运行 Gate。
