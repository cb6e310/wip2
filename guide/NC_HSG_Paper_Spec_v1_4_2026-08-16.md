# NC-HSG 小论文统一规格（综合整合版 v1.4，2026-08-16）
...
本文件是 `NC_HSG_Paper_Spec.md` 与 `NC_HSG_Paper_Spec_v1.md` 的逐项审阅、冲突裁决和统一规格，并吸收实现前的算法、随机化检验、风险校准、仓库治理与第三次输入准入审查。它是论文、实验和实现的共同合同；不是结果报告。文中所有【新】数值均必须在正式实验前冻结，所有【核】事实在核实前不得写成论文结论。v1.4 只更新 outcome-blind 的仓库事实、治理入口、ZuCo 许可证据与定向数据准入方案；不改变 v1.2/v1.3 的科学指标、阈值、Gate 或失败路线。

> **版本权威**：v1.4 是下一次 Codex 导入后应激活的 SPEC；导入前远程 `main@250ca9a67cf386784005a1edbbfe502d7df6f192` 的状态文件以 v1.3 为 active SPEC，但根 `AGENTS.md` 错指 v1.2，构成 `STATE_SPEC_CONFLICT`。下一 run 必须先修复并给 validator 增加回归检查。首次迭代所附的其他项目 ZIP 仍只作管理风格示例，其中的路线、状态、提交号和证据均不得导入本项目。
...
0. **V0 仓库【CLOSED】**：公开 `main@250ca9a67cf386784005a1edbbfe502d7df6f192` 已完成治理 bootstrap、环境同步、治理加固和输入发现；关闭 V0 不代表任何科学实现存在。
1. **G1 治理连续性【REOPENED】**：根 `AGENTS.md` 仍声明 v1.2，而 `AI_START_HERE.md`、`PROJECT_STATE.yaml`、`HANDOFF.md` 与实际状态均声明 v1.3；现有 validator 没检查 `AGENTS.md`，因此测试全绿仍漏过 `STATE_SPEC_CONFLICT`。下一 run 必须激活 v1.4、同步所有入口并增加正反回归测试。
2. **V1 数据【OPEN，范围已缩小】**：授权本地 ZuCo 2.0 路径和官方 OSF CC BY 4.0 已找到；仍须证明本地树对应官方节点，并从 NR 物理文件恢复 subject/session/task/block/trial、sampling/unit/reference、`EEG.chanlocs` 标签/坐标、event、stimulus identity、缺失单元与可复现 hash manifest。不得再要求用户另给许可证或重复全盘扫描。
...
**T2-CODEX：在 `wip2@250ca9a67cf386784005a1edbbfe502d7df6f192` 上修复 active-SPEC 冲突，并对现有 ZuCo 2.0 NR 做定向物理准入。**

本任务先激活 v1.4、把 `AGENTS.md` 与所有入口同步到同一 active SPEC，并让 validator 对入口漂移 fail closed；随后只访问已发现的本地 ZuCo 2.0 目录与官方 OSF/论文小型 metadata，复核 CC BY 4.0、读取官方 reader/任务材料及 MAT 的非结果 metadata，恢复 NR 的通道、采样、参考、事件和 trial 结构，并为实际拟使用的 18 个 NR summary 文件生成 SHA256 manifest。若六项数据准入条件全部满足，可在同一 run 完成 `S0_DATA_CARD`；否则只保留精确的机器可操作 blocker。不得重复扫描整个 `trust_align`、读取历史 held-out/test metric、下载数据或权重、选择 A、实现模型/null/schema、训练或运行 Gate。精确执行文本见交付 ZIP 根目录 `CODEX_NEXT_TASK.md`。
