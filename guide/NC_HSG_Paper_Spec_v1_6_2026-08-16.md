# NC-HSG 小论文统一规格（综合整合版 v1.6，2026-08-16）
...
本文件是 `NC_HSG_Paper_Spec.md` 与 `NC_HSG_Paper_Spec_v1.md` 的逐项审阅、冲突裁决和统一规格，并吸收实现前的算法、随机化检验、风险校准、仓库治理与第五次输入准入审查。它是论文、实验和实现的共同合同；不是结果报告。文中所有【新】数值均必须在正式实验前冻结，所有【核】事实在核实前不得写成论文结论。v1.6 只补齐 outcome-blind 的 ZuCo NR event-occurrence 与 summary↔preprocessed segment correspondence 合同，并修复证据缓存和条件式 data-card 分支；不改变 v1.2–v1.5 的科学指标、阈值、Gate 或失败路线。

> **版本权威**：v1.6 是下一次 Codex 导入后应激活的 SPEC。导入前远程 `main@c807a2e83fad02763193a6c1db81fd26db19fd97` 已正确激活 v1.5，并完成 run 006 的 schema-v2 cell 与 occurrence 修复。run 006 的 admission FAIL 与不生成 data card 的结论是安全的，但它只验证 `10→11`，没有把 `12→13` 纳入 349 个句子 occurrence，也没有验证 `sentenceData/rawData` 是否为 preprocessed `EEG/data` 的物理切片；其 source verdict 仍由脚本常量生成，all-PASS 分支反而退出。因此 run 006 保留为不可变历史，下一 run 只补齐这些合同，不扩大到模型、split、训练或 Gate。
...
0. **V0 仓库【CLOSED】**：公开 `main@c807a2e83fad02763193a6c1db81fd26db19fd97` 已完成治理 bootstrap、环境同步、治理加固、输入发现和两轮定向 NR 审计修复；关闭 V0 不代表数据已准入或任何科学实现存在。
1. **G1 治理连续性【CLOSED】**：run 005 建立动态 active-SPEC fail-closed 回归检查，run 006 已把全部入口同步到 v1.5。下一 run 只需按同一机制激活 v1.6，不得重做旧治理修复。
2. **V1 数据【OPEN，schema-v2 已修复，segment correspondence 待核】**：run 006 已证明 18×349=6,282、6,282/6,282 material occurrence、5,911 个 finite multisample、367 个 nonfinite placeholder 与 4 个 single-sample exclusion；许可证、27/27 OSF hash、105 通道、500 Hz、Cz acquisition reference 和 common-average processed reference 保留成立。当前 event 审计遗漏 `12→13` control-sentence occurrence，未证明 303 个 `10→11` 与 46 个 `12→13` 如何共同形成每名被试 349 个句子，也未做 summary `rawData` 对 preprocessed `EEG/data` 的流式逐段同一性检查。stored-array unit 仍 unresolved。下一 run 必须先补 event-occurrence 和 segment correspondence；不能等待式重跑 unit 搜索、重复 broad scan 或借由全局统计猜 layer/reference/unit。
...
**T4-CODEX：在 `wip2@c807a2e83fad02763193a6c1db81fd26db19fd97` 上补齐 ZuCo 2.0 NR event-occurrence 与 segment correspondence。**

本任务先激活 v1.6，并新增 `SPEC_V16_REVIEW → S0_ZUCO2_NR_SEGMENT_CORRESPONDENCE`；不得改写 run 005、run 006 或 schema-v1/v2 artifacts。随后只修改 targeted auditor、synthetic tests、schema-v3 准入 artifacts/report 与 active state：把 `10→11` 普通句和 `12→13` control 句统一解析为每名被试 349 个有序 occurrence；审计 `15` 为 control-question response 而非额外句子；以一个预先列明且全局唯一的 EEGLAB indexing convention，流式比较 event-defined `EEG/data` segment 与 summary `rawData`，不输出数值。只有 exact correspondence 才能绑定 summary layer/reference；unit 仍须独立证据。source verdict 必须来自可验证 evidence cache，不得由常量伪造。修复 all-PASS 分支，使六项全 PASS 时真实生成 data card，否则保持 BLOCKED。不得读取历史 held-out/test metric、下载数据或权重、选择 A、实现模型/null/schema、训练或运行 Gate。精确执行文本见交付 ZIP 根目录 `CODEX_NEXT_TASK.md`。
