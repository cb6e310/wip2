# RC-HSG 小论文统一规格（N2 common-phase sampler 版 v2.8，2026-08-24）
...
mechanism sampler；§27 是当前 N2 common-phase sampler 与下一动作的权威增量。Codex
只负责实现已冻结决定，
...
> **版本权威**：v2.8 是下一次 Codex 导入后应激活的 SPEC。导入前远程
> `main@06e3e5f9b5c720bbb29074ca1cae1109add5b1b9` 已完成 run 018，当前恰有 74 tasks、
> 41 DONE、8 SKIPPED、24 BLOCKED，唯一 READY/推荐任务为 owner=`CHATGPT_OR_AUTHOR`
> 的 `S0_N2_SAMPLER`。下一 run 只实现 §27：构造 CPU/float64-internal、float32-output、
> unpadded-valid-prefix-only 的 N2 common-phase transformer，对同一频率所有 105 channels
> 使用同一相位增量，DC/Nyquist 固定，K=199 replicate schedule 由 row key+replicate
> 确定；只用 deterministic synthetic fixtures 验证 PSD/cross-spectrum/covariance/mask/
> endpoint/amplitude diagnostics。不得打开任何真实 dataset、导入 A1/frontend、执行
> `GATE_R0`、生成 score/p-value、训练或解锁 test；成功后唯一 READY 必须转为
> `GATE_R0` 并停止，等待真实数据审计的新合同。
