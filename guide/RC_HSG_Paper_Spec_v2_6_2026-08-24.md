# RC-HSG 小论文统一规格（N1 block feasibility 版 v2.6，2026-08-24）
...
outer-train A1 admission；§25 是当前 N1 block-feasibility 与下一动作的权威增量。Codex
只负责实现已冻结决定，
...
> **版本权威**：v2.6 是下一次 Codex 导入后应激活的 SPEC。导入前远程
> `main@1c432a02f50cacda99359f630f14cfbfdfb439a1` 已完成 run 016，当前恰有 72 tasks、
> 37 DONE、8 SKIPPED、26 BLOCKED，唯一 READY/推荐任务为 owner=`CHATGPT_OR_AUTHOR`
> 的 `S0_N1_BLOCK_FEASIBILITY`。下一 run 只实现 §25：对 3,497 个 eligible outer-train
> rows 作一次共享 A1 spectral-tokenizer proxy scan，保留 44 short rows no-read，冻结
> train-fit power edges，构造 3,541-row metadata ledger并探测 199 个联合块内置换；不得读取
> cal/test、生成 donor EEG、实现 production N1/N2 sampler、训练、读取 outcome 或执行 Gate。
