# NC-HSG 小论文统一规格（综合整合版 v1.9，2026-08-23）
...
本文件是 `NC_HSG_Paper_Spec.md` 与 `NC_HSG_Paper_Spec_v1.md` 的逐项审阅、冲突裁决和统一规格，并吸收实现前的算法、随机化检验、风险校准、仓库治理、第六次输入准入审查、第七次 stimulus-identity 可执行性审查与第八次 grouping-policy 审查。它是论文、实验和实现的共同合同；不是结果报告。文中所有【新】数值均必须在正式实验前冻结，所有【核】事实在核实前不得写成论文结论。v1.9 接受 run 009 的 344-identity、58,996-pair outcome-blind diagnostic，并由 ChatGPT/作者在任何 split、训练或 outcome 读取前冻结最终分组规则；Codex 只负责按该规则生成身份与分组 artifacts，不得再决定阈值或顺手构造 split。v1.9 不改变 v1.2–v1.8 的科学指标、Gate、split 纪律、数据分析视图、unknown-unit policy 或失败路线。§12–§17 是不可改写的历史快照，§18 是当前状态与下一动作的权威增量。

> **版本权威**：v1.9 是下一次 Codex 导入后应激活的 SPEC。导入前远程 `main@1252d0a24b6d4785e7f550464586c95f54f3cfa3` 已正确激活 v1.8，并完成 run 009 的七 CSV source binding 与全对 diagnostic：349 task slots、344 exact identities、5 个 exact duplicate occurrence groups、58,996 unordered pairs、11 个 broad candidates；document/paragraph metadata 不可用，physical unit 仍为 `UNRESOLVED_RELEASE_NATIVE_AMPLITUDE`。下一 run 只完成冻结的 stimulus identity/group artifacts，成功后把 `S0_JOINT_SPLIT` 置为 READY 并停止；不得在同一 run 构造 Regime I/II split。
