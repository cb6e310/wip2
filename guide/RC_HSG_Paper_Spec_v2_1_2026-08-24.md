# RC-HSG 小论文统一规格（科学重构整合版 v2.1，2026-08-24）
...
本文件继承 v1.2–v2.0 的算法审查、风险纪律、仓库治理、输入准入、stimulus
identity/grouping 与 run-011 joint-split/population 证据，并采用 author-level RC-HSG
scientific redesign。它是论文、实验和实现的共同合同，不是结果报告。§12–§19 是
不可改写的历史/物理证据；§20 是当前科学问题、方法、Gate、任务图、A policy 与下一
动作的权威增量。Codex 只负责实现已冻结决定，不得重新研究 reference priority、
feature/model family、split、Gate 或 backbone。

> **版本权威**：v2.1 是下一次 Codex 导入后应激活的 SPEC。导入前远程
> `main@3b97fdc966b9b56d72287df619a80f6145d71189` 已完成 run 011 的 deterministic
> joint split 和 subject-macro population freeze，当前 sole READY 为 owner-only
> `S0_A_POLICY_REVIEW`。下一 run 只激活 RC-HSG redesign、物化 §20.10 已由作者冻结
> 的 A policy、把 `S0_A_INTERFACE` 置为 READY 后停止；不得实现 frontend、读取 EEG/
> outcome、解锁 test、训练或运行任何 Gate。
