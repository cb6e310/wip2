# RC-HSG 小论文统一规格（N1 mechanism sampler 版 v2.7，2026-08-24）
...
outer-train A1 admission；§25 是已完成的 N1 block-feasibility；§26 是当前 N1
mechanism sampler 与下一动作的权威增量。Codex
只负责实现已冻结决定，
...
> **版本权威**：v2.7 是下一次 Codex 导入后应激活的 SPEC。导入前远程
> `main@082ed4f72f1b8bbc18096a5f0caea2075b2783c4` 已完成 run 017，当前恰有 73 tasks、
> 39 DONE、8 SKIPPED、25 BLOCKED，唯一 READY/推荐任务为 owner=`CHATGPT_OR_AUTHOR`
> 的 `S0_N1_SAMPLER`。下一 run 只实现 §26：从已提交的 run-017 assignment metadata
> 在内存重建 3,481-row、180-block、K=199 的 exact joint permutations，逐 replicate
> 匹配冻结 hash/fixed-point evidence，并用 synthetic values 验证 real/pseudo-real
> 走同一 selection-aware callback。不得重跑 feasibility、读取任何 EEG、加载 donor
> embedding、构造 semantic candidate/reference score、计算 p-value、训练、执行 Gate 或
> 解锁 test；成功后唯一 READY 必须转为 `S0_N2_SAMPLER` 并停止，等待 N2 新合同。
