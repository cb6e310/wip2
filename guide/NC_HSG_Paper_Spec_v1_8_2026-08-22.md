# NC-HSG 小论文统一规格（综合整合版 v1.8，2026-08-22）
...
本文件是 `NC_HSG_Paper_Spec.md` 与 `NC_HSG_Paper_Spec_v1.md` 的逐项审阅、冲突裁决和统一规格，并吸收实现前的算法、随机化检验、风险校准、仓库治理、第六次输入准入审查与第七次 stimulus-identity 可执行性审查。它是论文、实验和实现的共同合同；不是结果报告。文中所有【新】数值均必须在正式实验前冻结，所有【核】事实在核实前不得写成论文结论。v1.8 接受 run 008 的 exclusion-tolerant data card，但修复下一任务的信息缺口：已提交 artifacts 只有 stimulus SHA256 与长度，不能据此计算编辑距离、语义 embedding、document/paragraph identity 或 paraphrase。v1.8 因此先授权读取 7 个已被 27/27 SHA256 绑定的小型 NR material CSV，生成不含原文的 source binding 与全对 similarity diagnostic；近重复阈值仍由后续 ChatGPT/作者审阅冻结，不交给 Codex 自主选择。v1.8 不改变 v1.2–v1.7 的科学指标、阈值、Gate、split 纪律或失败路线。§12–§16 是不可改写的历史快照，§17 是当前状态与下一动作的权威增量。

> **版本权威**：v1.8 是下一次 Codex 导入后应激活的 SPEC。导入前远程 `main@b72ed5ab9720b7a922f7d1c6d8681cb646c344ab` 已正确激活 v1.7，并完成 run 008 的 repository-only analysis view 与 data card：5,905 rows admitted、377 rows excluded，`full_release_diagnostic=FAIL` 与 `analysis_view_admission=PASS` 并存，NR3/NR5/NR6 active 叙述已修正，physical unit 仍为 `UNRESOLVED_RELEASE_NATIVE_AMPLITUDE`。下一 run 不做 split；它只完成 stimulus source binding 与 similarity diagnostic，为后续由 ChatGPT/作者冻结 grouping policy 提供 outcome-blind 分布证据。
