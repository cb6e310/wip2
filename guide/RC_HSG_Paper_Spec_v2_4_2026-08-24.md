# RC-HSG 小论文统一规格（A-path 泄漏审计版 v2.4，2026-08-24）
...
本文件继承 v1.2–v2.3 的算法审查、风险纪律、仓库治理、输入准入、stimulus
identity/grouping、run-011 joint-split/population 证据、run-012 RC-HSG activation 与
run-013 synthetic A-interface、run-014 bounded real-frontend evidence。
它是论文、实验和实现的共同合同，不是结果报告。§12–§19 是不可改写的历史/物理证据；
§20 是 RC-HSG 科学重构；§21 是 synthetic A-interface 与短片段路由；§22 是真实
frontend 自检与 leakage 分层；§23 是当前 early A-path leakage audit 与下一动作的权威
增量。Codex 只负责实现已冻结决定，
不得重新研究 reference priority、feature/model family、split、Gate、backbone、短片段
处理、真实数据审计抽样策略或 leakage 证据范围。

> **版本权威**：v2.4 是下一次 Codex 导入后应激活的 SPEC。导入前远程
> `main@dc105709563cf9eb216f1c28f82fdf754e7b0683` 已完成 run 014，当前恰有
> 70 tasks、33 DONE、8 SKIPPED、28 BLOCKED，唯一 READY/推荐任务为 owner=`CODEX` 的
> `S0_LEAKAGE_AUDIT`。下一 run 只实现 §23 的 static-code + committed-metadata +
> synthetic-fixture A-path audit，新增/完成 `SPEC_V24_REVIEW` 与 `S0_LEAKAGE_AUDIT`，
> 然后以唯一 READY `S0_A1_ADMISSION` 停止；不得打开 production HDF5、读取任何新真实
> EEG/outcome、执行 full admission、训练、后期 method leakage audit 或任何 Gate。
