# RC-HSG 小论文统一规格（A 接口冻结版 v2.2，2026-08-24）
...
本文件继承 v1.2–v2.1 的算法审查、风险纪律、仓库治理、输入准入、stimulus
identity/grouping、run-011 joint-split/population 证据与 run-012 RC-HSG activation。
它是论文、实验和实现的共同合同，不是结果报告。§12–§19 是不可改写的历史/物理证据；
§20 是 RC-HSG 科学重构；§21 是当前 A-interface、短片段路由、任务图和下一动作的权威
增量。Codex 只负责实现已冻结决定，不得重新研究 reference priority、feature/model
family、split、Gate、backbone 或短片段处理策略。

> **版本权威**：v2.2 是下一次 Codex 导入后应激活的 SPEC。导入前远程
> `main@91997faa1de1616d1eb662cd36edc1547613206d` 已完成 run 012，当前恰有
> 67 tasks、29 DONE，唯一 READY/推荐任务为 owner=`CODEX` 的 `S0_A_INTERFACE`。
> 下一 run 只实现 §21 冻结的 clean-room A interface、生成 outcome-blind metadata
> eligibility overlay、把 `S0_A1_FRONTEND` 置为唯一 READY 后停止；不得读取真实 EEG
> 数值、训练、执行 real-data frontend admission、full leakage audit 或任何 Gate。
