# RC-HSG 小论文统一规格（Gate R0 outcome-blind real-audit 版 v2.9，2026-08-24）

> **v2.9 outcome-blind Gate R0 real-audit freeze.** 本版本完整继承 v2.8/run-019
> 已实现的 N2 multivariate common-phase sampler、synthetic preservation、N1
> mechanism-only 状态、A1 full outer-train admission、early leakage firewall、joint split、
> route unlock 与 test lock；以本文 §28 冻结 `GATE_R0` 唯一允许的真实值审计：只读
> `train_fit|inner_val`、每个 3,497 eligible row 精确一次、cal/test/text/outcome 全部零读，
> 以 train-fit 拟合、inner-val 判定的 balanced real/N2 classifier、A1-token nuisance transfer、
> bounded/full replicate numerical audit、amplitude/endpoint/mask/coverage tolerances 和
> PASS/FAIL 论文路线。发生冲突时，§28 对 Gate R0 与下一动作具有最高权威；§27 的 N2
> transform law 与 synthetic evidence、§26 的 N1 mechanism boundary、§§20–25 的其余
> 决定继续有效。本版本不读取 semantic outcome、stimulus text、calibration/test arrays 或
> test identity，不训练语义模型，不生成 reference score/p-value，不执行 Gate R/C/H 或
> Mechanism A。Gate R0 只决定 reference integrity/admissibility，不证明性能提升或机制发现。

> **v2.8 synthetic-only N2 common-phase sampler freeze.** 本版本完整继承 v2.7/run-018
> 已实现的 N1 mechanism-only sampler、run-017 DEGRADED_COVERAGE 后果、A1 full
> outer-train admission、early leakage firewall、joint split、route unlock 与 test lock；以本文
> §27 冻结 `S0_N2_SAMPLER` 的 multivariate common-phase Fourier 数学/数值/API 合同、
> synthetic-only preservation diagnostics、零真实数据读取、run-018 provenance typo 的追加
> 更正和 run-019 状态迁移。发生冲突时，§27 对 N2 sampler 与下一动作具有最高权威；
> §§20、26 的 reference priority/claim boundary/N1 facts 和 §§14–25 的物理证据保持有效。
> 本版本只证明 sampler 实现符合冻结变换，不决定真实 EEG 上的 N2 admissibility；real-vs-
> surrogate classifier、nuisance/endpoint/amplitude/coverage 与 N2 primary 准入仍由下一轮
> outcome-blind `GATE_R0` 决定。本版本不打开真实 EEG、文本、outcome 或 test identity，
> 不加载 A1、生成 reference score/p-value、训练或执行任何 Gate。

> **v2.7 outcome-blind N1 mechanism-sampler freeze.** 本版本完整继承 v2.6 已完成的
> run-017 N1 block-feasibility evidence、3,541-row assignment ledger、3,481-row evaluable
> population、199 个联合块内置换摘要、A-path leakage firewall、joint split、test lock 与
> RC-HSG 科学重构；以本文 §26 冻结 `S0_N1_SAMPLER` 的最小 index-only production
> sampler、与 run-017 的逐 replicate parity、selection-aware pseudo-real 回调边界、
> no-EEG/no-score/no-p-value 证据和 run-018 状态迁移。run 017 的结论保持
> `DEGRADED_COVERAGE`：N1 永久不得成为 primary fallback，只保留
> mechanism/robustness；论文 primary reference 立即转向 N2。发生冲突时，§26 对本轮
> N1 sampler 与论文下一步具有最高权威；§25 的 assignment/随机化律/覆盖结论、§24 的
> A1 admission、§23 的 early leakage PASS、§§20–21 的 RC-HSG/A-interface 决定和
> §§14–19 的物理事实保持有效。本版本不读任何 EEG、文本或 outcome，不生成 embedding、
> candidate、reference score 或 p-value，不执行任何 Gate，也不解锁 test。

> **v2.6 outcome-blind N1 block-feasibility freeze.** 本版本完整继承 v2.5 已通过的
> 3,497-row full outer-train A1 admission、44-row short forced-L0 路由、early A-path
> leakage firewall、joint split、test lock 与 RC-HSG 科学重构；以本文 §25 冻结
> `S0_N1_BLOCK_FEASIBILITY` 的 EEG-observable length strata、train-fit-frozen
> dimensionless power proxy/edges、单次 outer-train proxy scan、singleton/coverage
> ledger、K=199 metadata-only joint-permutation probe、三分支 decision 与 run-017 状态迁移。
> 发生冲突时，§25 对 N1 feasibility 具有最高权威；§24 的 admission evidence、§23 的
> early leakage PASS、§§20–21 的 RC-HSG/A-interface 决定和 §§14–19 的物理事实保持有效。
> 本版本不生成 donor EEG、reference score、semantic outcome、Gate 结论或 test value。
...
本文件继承 v1.2–v2.5 的算法审查、风险纪律、仓库治理、输入准入、stimulus
identity/grouping、run-011 joint-split/population 证据、run-012 RC-HSG activation 与
run-013 synthetic A-interface、run-014 bounded real-frontend、run-015 early leakage、
run-016 full outer-train admission evidence。
它是论文、实验和实现的共同合同，不是结果报告。§12–§19 是不可改写的历史/物理证据；
§20 是 RC-HSG 科学重构；§21 是 synthetic A-interface 与短片段路由；§22 是真实
frontend 自检与 leakage 分层；§23 是 early A-path leakage audit；§24 是 full
outer-train A1 admission；§25 是已完成的 N1 block-feasibility；§26 是已完成的 N1
mechanism sampler；§27 是已完成的 N2 common-phase sampler；§28 是当前 Gate R0
outcome-blind real audit 与下一动作的权威增量。Codex
只负责实现已冻结决定，
不得重新研究 reference priority、feature/model family、split、Gate、backbone、短片段
处理、功率代理、bin、coverage threshold、置换算法、leakage 证据范围或失败路由。

> **版本权威**：v2.9 是下一次 Codex 导入后应激活的 SPEC。导入前远程
> `main@4fa6fadc8bdee0d163acc8bf9ee48aeac4d3095d` 已完成 run 019 及状态字符串纠正，
> 当前恰有 75 tasks、43 DONE、8 SKIPPED、23 BLOCKED，唯一 READY/推荐任务为
> owner=`CHATGPT_OR_AUTHOR` 的 `GATE_R0`。下一 run 只执行 §28：用已经准入的 outer-train
> 真实 EEG 对 N1/N2 reference integrity 做 outcome-blind 审计。真实读取 allowlist 精确为
> 3,497 个 `train_fit|inner_val` eligible arrays、每 row 一次；44 short、全部 cal/test、文本、
> semantic outcome 与 test identity 零读。成功完成审计后，无论科学 PASS/FAIL，唯一 READY
> 转为 owner=`CHATGPT_OR_AUTHOR` 的 `S0_SEMANTIC_ITEM` 并停止；不得顺带实现 schema、
> candidate、reference feature、reliability model、calibration、route lock 或任何后续 Gate。
...
## 25. v2.6：outcome-blind N1 block feasibility 与 run 017 合同

### 25.1 run-016 验收、当前仓库事实与治理修复

独立复核基线为 clean `main@1c432a02f50cacda99359f630f14cfbfdfb439a1`；
`HEAD=origin/main`、porcelain 为空。validator/status PASS，恢复状态恰为 **72 tasks、
37 DONE、8 SKIPPED、26 BLOCKED、唯一 READY `S0_N1_BLOCK_FEASIBILITY`**，owner=
`CHATGPT_OR_AUTHOR`；route 未锁，test 仍为 `LOCKED_UNTIL_ROUTE_LOCK`。

run 016 的 full-admission evidence 予以接受：3,541-row ledger 覆盖 3,497 eligible 与
44 short forced-L0 rows；run 014 的 107-row/1,452-window panel 被复用且未重读，run 016
单次读取其余 3,390 rows/34,293 windows；累计 35,745 windows、18 subjects/files，所有
source dtype 为 float64，full frontend 检查 PASS。服务器 run record 报告 full discovery
198/198 no skip；独立 review 环境可复跑 project-memory 55/55，缺 h5py 时不得把未运行的
admission suite 冒充本地 PASS。

当前治理存在一个必须随 v2.6 修复的 stale sentence：`GATE_R0.blocked_reason` 仍称 full
frontend/early leakage 不完整且 B_V9 active。物理事实恰相反：early leakage 与 full
outer-train admission 已完成，B_V9 已 closed；当前 Gate R0 只因 B_V4 下的 N1/N2 family
尚未完成而 BLOCKED。run 017 必须修正文案，但不得把 Gate R0 迁为 READY 或 PASS。

### 25.2 本任务的科学问题、边界与结论上限

N1 的目的不是从结果中寻找“最容易置换”的块，而是先回答：在固定
`subject × session × EEG-length × EEG-power` nuisance 条件和 split firewall 下，是否存在
足够覆盖的联合块内置换群。run 017 是 **outcome-blind protocol feasibility**：只冻结
power edges、block assignments 与 index-level permutation probe；不读取 stimulus text、
semantic label、prediction、calibration/test outcome 或历史 metric。

v2.5 禁止重复读 outer-train 是 run-016 admission scope 的停止线；本 §25 为新的、独立的
band-power feasibility 目的，明确授权对 **全部 3,497 eligible outer-train arrays 各读取
一次**。这是无法从 run-016 metadata-only ledger 恢复的必要统计量；禁止借机重跑 full
encoder、保存 tokens/embeddings 或扩大到 cal/test。44 short rows 保留在总体与 ledger 中，
但仍不得 dereference。

本任务最多允许写：

```text
N1_OUTER_TRAIN_BLOCK_FEASIBILITY_PASS
N1_OUTER_TRAIN_BLOCK_FEASIBILITY_DEGRADED_COVERAGE
N1_OUTER_TRAIN_BLOCK_FEASIBILITY_FAIL
```

不得写 N1 Gate R0 PASS、N1 primary fallback admitted、exact randomization validity on cal/test、
semantic/reference utility 或 performance。即使 outer-train PASS，§20.4 要求的所有 legal
pre-test roles 仍缺未读 calibration coverage，因此 N1 primary fallback 只能保持
`PENDING_CAL_COVERAGE_AND_GATE_R0`。

### 25.3 固定输入与 read firewall

production builder 的 baseline 固定为 `1c432a02f50cacda99359f630f14cfbfdfb439a1`。读取前
必须验证下列完整 SHA256；active v2.6 SPEC 与新 script 的 actual hash 另写入 output：
...
新增 `scripts/audit_n1_block_feasibility.py`。它不得直接 import `h5py`，不得实现第二套
reader 或频谱路径，也不得修改 frozen validator/A code。先完成零真实值 preflight：
3,541/3,497/44/35,745 counts、key union/disjoint、roles、18 subjects/files、session=1、task=NR、
run-016 all-row PASS、output roots 与 test lock 全一致；随后 lazy-import exact-hash
`scripts/validate_a1_frontend.py`，只复用：

```text
_dataset_files, _read_raw, _row_key, METADATA,
PRODUCTION_DATASET_ROOT, NativeSpectralA1
```

对 eligible ledger rows 在内存构造 `source_dataset_read=true` 的 audited-loader record；
`allowed_keys` 必须精确等于 3,497 eligible outer-train keys。真实 scan order 固定为：

```text
(subject, session, length_bin, role, slot, occurrence_id)
```

每 row 只读一次、处理后立即释放；short/cal/test dereference 必须为零。不得 preload、resume、
重试失败 row、写 value cache 或 mid-run 改算法。source/finite/hash/scope 失败报告稳定 row key 与
error code，不能写 raw value 或绝对 path。

### 25.4 EEG-observable length 与共享 A1 power proxy

长度只使用已经在 run 014 前由 metadata 冻结的完整窗口数，不使用 stimulus/text length、
raw text、outcome 或实验 block：
...
```text
RC_HSG_N1_A1_LOG_RELATIVE_BANDPOWER_MEDIAN_V1
```

它不是物理单位功率，也不解析 microvolt。对 `_read_raw` 返回的 CPU float32
`trial[1,105,T]`，实例化 CPU `NativeSpectralA1(20260824).eval()`，以 frozen metadata 调用
`_validate`，随后只调用 exact `_spectral_tokens(trial[0], T)`；禁止调用 full `forward`、
projection 或 Transformer。tokens 必须为 finite `[window_count,840]`，再视为
`[window_count,105,8]` 的 frozen log-relative-bandpower。

把全部 `N=window_count*105*8` 个 float32 token 值展平；N 在本合同下总为偶数。以 1-based
`k=N/2` 与 `k+1` 的两个 order statistics 定义：

\[
p_i=\frac{x_{(k)}+x_{(k+1)}}{2}.
\]

实现必须用 CPU `torch.kthvalue` 取得两个 float32 值，再转 Python float，以
`math.fsum([a,b])/2.0` 形成 float64 scalar。它是单位/全局幅值不敏感的 nuisance proxy，
不是 signal quality、EEG evidence 或论文 outcome。row-level `p_i`、token、频谱、amplitude、
waveform、embedding 与其 hash 一律不得落盘。

### 25.5 train-fit-frozen power edge 与不可借样本规则

power edge cell 精确为 `(subject, session, length_bin)`；role 不进入 edge fitting。对每 cell
只使用 `role=train_fit` 的 proxy。最小训练 cell 数固定为：

```text
MIN_TRAIN_ROWS_PER_POWER_EDGE = 4
```

若 `n_train<4`，edge status=`INSUFFICIENT_TRAIN_CELL`，该 cell 在 train-fit/inner-val 的所有
rows 都标 `N1_NOT_EVALUABLE_POWER_EDGE_UNAVAILABLE`；不得借相邻 length、其他 subject、
inner-val、cal 或 test 来补 edge。否则按与 §25.4 相同的 exact median 算法计算 edge，保存为
Python `float.hex()` 的 `edge_float64_hex` 字符串，并冻结：
...
ties 固定进入 `P_LOW`；记录 train low/high/tie counts，但不得搜索 alternate quantile、抖动
ties、改为 tertile、合并 bins 或根据 coverage 调 edge。edge 一旦由 train-fit 冻结，原样应用
于 inner-val；本 run 不应用到 cal/test。每个 edge cell ID 为：

```text
"n1pc_v1_" + SHA256(
  b"RC_HSG_N1_POWER_CELL_V1\0" +
  ASCII(subject + "\t" + session + "\t" + length_bin)
).hexdigest()
```

### 25.6 N1 block、singleton 与 3,541-row ledger

科学 block key 固定为 `(subject,session,length_bin,power_bin)`；执行 scope 另加 `role`，因此
实际 permutation block 为：

```text
(role, subject, session, length_bin, power_bin)
```

`role` 只能为 `train_fit|inner_val`，用来禁止跨 split donor；`task=NR` 与 session=1 虽在
ledger 验证，但 task 因全局常量不增加 key；source experimental `block=1..7` 明确**不进入**
N1 key，因为原始预注册 nuisance contract 没有该维度，加入只会事后碎片化。不得跨 role、
subject、session、length 或 power 借 donor，也不得相邻 bin borrowing。

block ID 固定为：

```text
"n1b_v1_" + SHA256(
  b"RC_HSG_N1_BLOCK_V1\0" +
  ASCII(role + "\t" + subject + "\t" + session + "\t" + length_bin + "\t" + power_bin)
).hexdigest()
```

block size=1 的 row 标 `N1_NOT_EVALUABLE_SINGLETON`；不借 donor、不强制 derangement、也不
删除该 row。block size>=2 才是 `N1_EVALUABLE`。44 short rows 标
`N1_NOT_EVALUABLE_SHORT_FORCED_L0`；edge unavailable rows 使用上节状态。新增
`artifacts/nulls/n1_block_assignment_v1.jsonl`，canonical order
`(subject,slot,occurrence_id)`，每 row fields/order 精确为：
...
ledger 不含 proxy/edge numeric value、EEG/token/embedding、stimulus ID/text、outcome、
prediction、metric、donor key 或 permutation map。edge values 只在 summary YAML 的 edge
table 中保存，供未来 sampler 精确复用。

coverage 必须同时报告：

1. `eligible_conditional_coverage = evaluable / eligible`；
2. **primary** `population_coverage = evaluable / all outer-train rows`，分母包含 short、
   edge-unavailable 与 singleton rows；
3. overall、by role、by subject×role、by length/power、exclusion-reason counts；
4. block-size distribution、singleton blocks/rows、minimum/median/maximum block size。

§20.4 的 0.90 判据使用第 2 项，避免通过排除 forced-L0/困难 rows 虚高覆盖。

### 25.7 K=199 index-only joint-permutation probe

probe policy ID：

```text
RC_HSG_N1_JOINT_PERMUTATION_PROBE_V1
```

只对 `N1_EVALUABLE` rows 生成 index mapping，不读 donor EEG、不调用 frontend、不生成
reference score。canonical row key 为 ASCII：

```text
subject + "\t" + zero-padded-6-digit-slot + "\t" + occurrence_id
```

对 replicate `r=1..199` 与每个按 block ID 排序的 block：recipient 按 row key 升序；每个
candidate donor 的排序 digest 为：

```text
SHA256(
  b"RC_HSG_N1_FEASIBILITY_V1\0PERM\0" +
  uint16_be(r) + b"\0" + ASCII(block_id) + b"\0" + ASCII(donor_row_key)
)
```

donor 按 `(digest,row_key)` 升序并与 recipients 按位置 zip。每个 replicate 必须对每个 block
构成一次完整 bijection。fixed point 是 recipient==donor，**允许且必须报告**；不得拒绝或
重抽 fixed point，因为 derangement 会改变冻结的随机化群。

每个 replicate 的 joint mapping hash 只在内存按以下 lines 的 ASCII 全局升序 bytes 计算：

```text
replicate<TAB>block_id<TAB>recipient_row_key<TAB>donor_row_key<LF>
```

artifact 只保存 199 项 `{replicate_id,joint_mapping_sha256,evaluable_rows,fixed_points}`，不保存
mapping line、recipient/donor relation 或 surrogate hash。必须报告：199 joint hashes 的 unique
count/rate、每 replicate fixed-point count/rate、overall/by-role/by-subject fixed-point rate、
每 row 在 199 次中的 distinct-donor-count distribution、bijection/cross-role/cross-block
violations。fixed-point rate 是诊断，不设结果驱动 PASS cutoff；唯一性阈值固定为
`199/199 = 1.0`。

### 25.8 预冻结 decision rule 与失败路由

先定义 structural PASS，必须同时满足：

```text
all fixed input/hash/scope checks PASS
3,497 eligible arrays read exactly once; short/cal/test read zero
all proxy/token/edge/bin/block outputs finite and deterministic
at least one evaluable block in every subject×role
0 cross-role, cross-subject, cross-session, cross-length, cross-power mappings
199/199 replicates are within-block bijections
199/199 joint mapping hashes are unique
three rendered copies of every output are byte-identical
no value/text/outcome/donor-map/cache emission
```

完成的 feasibility decision 只能按下列机械规则产生：

```text
IF structural PASS AND min subject×role population_coverage >= 0.90:
    decision = PASS
    evidence_label = N1_OUTER_TRAIN_BLOCK_FEASIBILITY_PASS
    primary_fallback_status = PENDING_CAL_COVERAGE_AND_GATE_R0
    next = S0_N1_SAMPLER (CHATGPT_OR_AUTHOR; new exact contract required)

ELSE IF structural PASS:
    ...
    next = S0_N1_SAMPLER (mechanism/robustness only; new exact contract required)

ELSE:
    decision = FAIL
    evidence_label = N1_OUTER_TRAIN_BLOCK_FEASIBILITY_FAIL
    primary_fallback_status = N1_FAMILY_REJECTED
    next = S0_N2_SAMPLER (CHATGPT_OR_AUTHOR; new exact contract required)
```

hash/read/source/tensor/output/test failure不是科学 `FAIL`，而是
`N1_BLOCK_FEASIBILITY_BLOCKED`：不得完成 task、不得产生 canonical PASS/FAIL artifact、不得
迁移 next task。只要 structural probe 能正确完成，coverage 低只能按上表 DEGRADED，Codex
不得改 bins/edge/min-cell/0.90 阈值挽救。

### 25.9 implementation、产物与确定性

函数 API 固定为：

```python
audit_n1_block_feasibility(
    project_root: Path,
    dataset_root: Path,
    canonical_output_root: Path,
    verification_output_roots: tuple[Path, Path],
...
production CLI 只允许 `--output-root --verification-root-a --verification-root-b`；verification
A/B 必须提供且 repository-external。不得暴露 dataset、role、bin、edge、minimum cell、seed、
K、threshold、device、resume 或 hash override。tests 可在函数层注入 isolated synthetic root
并设置 `enforce_frozen_expectations=False`，production CLI 不暴露。
...
YAML 顶层 canonical key order：

```text
schema_version, artifact, spec_version, baseline_commit, task, policy_id,
evidence_scope, input_artifacts, authorized_scope, power_proxy_contract,
length_bin_contract, power_bin_contract, block_contract,
permutation_probe_contract, acceptance_thresholds, acceptance_counts,
power_edges, coverage, block_size_distribution, permutation_probe,
decision, implementation, prohibited, safety, downstream_boundary
```
...
真实 arrays 只 scan 一次；scan 完成后在内存构造 ledger/YAML/report，同一 bytes 依次原子写入
verification A、verification B、canonical root，三份逐文件一致；不得为 determinism 重扫。
stable exception 为 `N1BlockFeasibilityError(RuntimeError)`；前缀至少：

```text
N1_FEASIBILITY_INPUT_MISMATCH
N1_FEASIBILITY_SCOPE_MISMATCH
N1_FEASIBILITY_SOURCE_BLOCKED
N1_FEASIBILITY_PROXY_BLOCKED
N1_FEASIBILITY_STRUCTURAL_BLOCKED
N1_FEASIBILITY_OUTPUT_FAILURE
```

`safety` 至少记录 scan attempts=1、eligible read=3,497、short/cal/test read=0、CPU tokenizer
only、full encoder=false、row proxy persisted=false、donor EEG read=false、donor map persisted=false、
text/outcome read=false、training/update=false、test locked。report 必须解释 PASS/DEGRADED/FAIL
边界、coverage 分母、fixed points 为什么不被删除、cal/test 未验证限制。

### 25.10 tests 与 run-017 acceptance

synthetic tests 至少覆盖：fixed hashes/schema/count/key joins；length boundaries 1/4/5/16/17；
exact A1-tokenizer-only AST；proxy two-middle order statistics；positive scale invariance；edge 仅
train-fit fit；`n_train=3/4` boundary；hex edge roundtrip；`<=` tie rule；禁止相邻/跨 subject/
inner-val/cal edge borrowing；short no-read；singleton exclusion；population denominator 含 short；
coverage 0.90 boundary；199 hash-sort bijections；fixed points保留；199 unique threshold；tampered
role/block/recipient/donor rejection；no donor map/value/text/cache output；eligible exact-once；
cal/test/short zero-read；atomic failure；symlink/path escape；unknown CLI；one-scan three-render identity。

服务器最低验证集：

```text
.venv/bin/python -m unittest discover -s tests -p 'test_audit_n1_block_feasibility.py'
.venv/bin/python -m unittest discover -s tests -p 'test_admit_a1_outer_train.py'
.venv/bin/python -m unittest discover -s tests -p 'test_validate_a1_frontend.py'
.venv/bin/python -m unittest discover -s tests -p 'test_native_spectral_a1.py'
.venv/bin/python -m unittest discover -s tests -p 'test_build_a_interface_contract.py'
.venv/bin/python -m unittest discover -s tests -p 'test_audit_a_path_leakage.py'
...
全部必须实际 PASS/no skip；记录真实 test 数，不预填。完成后审 diff 不得含 raw value、proxy、
token、donor map、text、outcome、cache 或 large binary，写 immutable run 017，commit/push，并
证明 `origin/main==HEAD` 且 clean。任何测试/validator/state/hash mismatch 不得通过修改
scientific constants迎合。

### 25.11 task/blocker/state migration
...
SPEC_V26_REVIEW
-> S0_N1_BLOCK_FEASIBILITY
-> branch-specific owner=CHATGPT_OR_AUTHOR READY task
-> STOP
```

新增并完成 `SPEC_V26_REVIEW`，prerequisites=`[SPEC_V25_REVIEW]`；更新
`S0_N1_BLOCK_FEASIBILITY.produces` 为 §25.9 的 script/test/ledger/YAML/report/run，完成时 evidence
scope 使用精确字符串。无论 decision 为 PASS/DEGRADED/FAIL，`S0_N1_BLOCK_FEASIBILITY=DONE`；
只有 execution BLOCKED 才保持未完成。

同时强制修复：

```text
GATE_R0.blocked_reason =
  N1/N2 reference families remain incomplete under B_V4; early A-path leakage and
  full outer-train A1 admission are complete, and B_V9 is closed.

S0_N2_SAMPLER.blocked_reason =
  B_V4 N2 common-phase construction and admissibility tolerances require a new
  ChatGPT/author-frozen contract; real outer-train A1 admission is complete.
```

不得改 Gate outcome、route lock 或 test lock。状态分支：

| decision | task counts | 唯一 READY | N1 后果 |
|---|---|---|---|
| PASS | 73 total / 39 DONE / 8 SKIPPED / 25 BLOCKED / 1 READY | `S0_N1_SAMPLER` | fallback 仍待 cal coverage + Gate R0 |
| DEGRADED_COVERAGE | 同 PASS | `S0_N1_SAMPLER` | 只保留 mechanism/robustness；不得成为 primary fallback |
| FAIL | 73 total / 39 DONE / 9 SKIPPED / 24 BLOCKED / 1 READY | `S0_N2_SAMPLER` | `S0_N1_SAMPLER=SKIPPED`, reason=`N1_BLOCK_FEASIBILITY_FAIL_V26` |

PASS/DEGRADED 时 `S0_N1_SAMPLER` 只变 READY、owner=`CHATGPT_OR_AUTHOR`，root next task 明写
等待下一版 sampler exact contract；FAIL 时 `S0_N2_SAMPLER` 同理。PROJECT_STATE 共同更新：

```text
project.spec_version       v2.6
project.spec_path          guide/RC_HSG_Paper_Spec_v2_6_2026-08-24.md
project.baseline_commit    1c432a02f50cacda99359f630f14cfbfdfb439a1
project.reviewed_commit    1c432a02f50cacda99359f630f14cfbfdfb439a1
last_completed_task        S0_N1_BLOCK_FEASIBILITY
last_run                   runs/2026-08-24_017_n1_block_feasibility.md
execution.stage/status     stage_0/READY
route.locked               null
test_status                LOCKED_UNTIL_ROUTE_LOCK
```

`repository_status` 与 `recommended_next_task` 按 decision 分支写精确事实。同步 AGENTS、
AI_START_HERE、HANDOFF、CODEX_NEXT_TASK、TASKS、PROJECT_STATE、implementation matrix、snapshots、
validator/status/tests；旧 SPEC/review/run、split、A code、validator、run-014–016 artifacts
byte-immutable。

### 25.12 对论文前进方向的更新

run 017 把论文从“输入/前端可用”推进到“第一类 matched reference 的结构可用性已量化”。
但 RC-HSG 的 primary reference priority 仍是 §20.4 的 N2；N1 的价值是严格块内随机化
mechanism/robustness，以及 N2 失败时的预注册 fallback。因而：

- PASS：下一轮先冻结 production N1 sampler 合同，再研究 N2 common-phase/tolerances；不得
  把 outer-train PASS 写成 N1 primary admitted；
- DEGRADED：N1 仍可做带 coverage 限制的 mechanism/robustness，但从 primary fallback 永久
  移除；论文主线不死亡，因为 N2 本来就是 primary；
- FAIL：删除 N1 family，直接转 N2 contract；只要 N2 最终 Gate R0 PASS，RC-HSG 主线仍可
  前进；
- N2 也失败时才按 §20.9 转 ordinary hierarchical selective generation。

任何分支都不允许跳到 semantic outcomes、reference utility、calibration 或 test。最近的
合法关键路径仍是：`N1 feasibility -> N1/N2 implementation -> Gate R0 -> schema/candidates ->
reference features/reliability -> calibration/method leakage -> route lock -> Gates`。

### 25.13 run-017 硬停止线

本 run 只授权 3,497 eligible outer-train arrays 的一次 A1 spectral-tokenizer power-proxy scan
和 index-only feasibility probe。禁止 dereference 44 short、任何 cal/test array；禁止保存
row proxy、power spectrum、tokens、embedding、waveform、donor map/hash relation、value cache；
禁止 full A encoder、training/backward/optimizer、unit inference、A/split/window/short-route 修改；
禁止 production N1 donor generation、N2、F/schema/candidates/reference scores/reliability/
calibration/baseline/method leakage/Gate，或解锁 test。

Codex 只实现 §25 已冻结合同、测试、证据、状态迁移与 run record。成功后必须在分支对应的
owner=`CHATGPT_OR_AUTHOR` 唯一 READY task 处 STOP/push。任何需要改变 proxy、edge、bin、
minimum cell、coverage denominator/threshold、permutation law、decision route 或 read scope 的
情况，停止回传 ChatGPT/author，不得自主裁决。

---

## 26. v2.7：N1 mechanism-only index sampler 与论文转向 N2 的精确合同

### 26.1 当前远程事实与 run-017 接受结论

本版本审查的 clean remote baseline 固定为：

```text
branch                         main
HEAD = origin/main             082ed4f72f1b8bbc18096a5f0caea2075b2783c4
commit                         feat: audit N1 block feasibility
tasks                          73 total / 39 DONE / 8 SKIPPED / 25 BLOCKED / 1 READY
sole READY                     S0_N1_SAMPLER
owner                          CHATGPT_OR_AUTHOR
route.locked                   null
test_status                    LOCKED_UNTIL_ROUTE_LOCK
B_V4                           active
run-017 decision               DEGRADED_COVERAGE
run-017 structural_status      PASS
primary_fallback_status        INELIGIBLE_DUE_TO_OUTER_TRAIN_COVERAGE_BELOW_0_90
```

validator、status、`git diff --check` 均通过；服务器 run 017 记录 full discovery
`219/219`、project-memory `59/59`、新 feasibility suite `17/17`，全部 no skip。审查阶段又用
独立实现从 frozen assignment 重建全部 199 个 mappings，199 个 joint hashes、每 replicate
fixed-point count、总 fixed points `35,529` 均与 committed YAML 完全一致。

run 017 的关键量化事实为：

```text
outer-train ledger rows                         3,541
A1-eligible rows / exact reads                  3,497 / 3,497 exactly once
N1-evaluable rows                               3,481
populated role-scoped blocks                    192
evaluable blocks                                180
singleton blocks / rows                         12 / 12
short forced-L0                                 44
power-edge unavailable                          4
total non-evaluable                             60
overall eligible-conditional coverage           0.995424649700
overall population coverage                     0.983055634002
minimum subject×role population coverage        0.777777777778
joint mapping unique hashes                     199 / 199
bijection / cross-block violations              0 / 0
fixed points total / overall rate               35,529 / 0.051289195186
per-replicate fixed-point range                 145..214
```

最低 coverage 来自 `YAK × inner_val`：36 个 population rows 中 28 个 evaluable、7 个 short、
1 个 edge-unavailable，population coverage=`28/36=0.777777777778`；`YAK × train_fit` 为
`118/143=0.825174825175`，其中 24 个 short、1 个 edge-unavailable。由此可知降级主要来自
既已冻结的 forced-L0 population denominator，而不是置换双射、跨块泄漏或 hash
determinism 失败。不得重新分 bin、借 edge、删除 short/singleton 或改变分母/0.90 阈值来
挽救 N1 primary 地位。

### 26.2 本轮科学裁决与投入上限

N2 common-phase surrogate 继续是 §20.4 冻结的 **primary reference family**。N1 仅保留：

1. 严格 recipient→donor index 映射的 mechanism/robustness family；
2. 将来 Gate R0 后、且 candidate/path 对 real 与每个 pseudo-real 完全对称重算时的
   selection-aware randomization reference；
3. 解释 reference calibration 是否依赖严格块内 trial reassignment 的敏感性分析。

N1 永久不得再写成 primary、primary fallback、全 population coverage reference 或 N2 失败
后的 primary rescue。为控制研究投入，本轮只把 run-017 已证明可重建的 index law 做成最小
reusable module；完成即转向 `S0_N2_SAMPLER`。不得为 N1 增加 EEG scan、embedding cache、
semantic pipeline、候选库、score、p-value、训练或结果图。

### 26.3 固定输入与 no-read 边界

run 018 必须在第一次写文件前验证以下 exact inputs；任一 mismatch 以
`STATE_SPEC_CONFLICT` 停止，不得重算 run 017：

| label | path | SHA256 |
|---|---|---|
| v2.6 SPEC | `guide/RC_HSG_Paper_Spec_v2_6_2026-08-24.md` | `174f0ee08870cc045a75336d3fc7138c97a99e78e5adfb109aed74b5c5144aaa` |
| v2.6 review | `artifacts/spec_review/rc_hsg_v26_n1_block_feasibility_review.md` | `b44e0a97c57d8e51e3e8365c56781c88b37328acc28d21f40f070e876d421e87` |
| assignment | `artifacts/nulls/n1_block_assignment_v1.jsonl` | `d0acc5e5fe78bc36a69cb04b6f605983c675e49a764538ae1665f86a28acee04` |
| feasibility YAML | `artifacts/nulls/n1_block_feasibility.yaml` | `90a6178100f507299e12223d15291699aad84e4b58bb52e29843dbf99ee6f771` |
| feasibility report | `reports/n1_block_feasibility.md` | `5bf77b8282d0938d59104b5e4e615c30c3b4fbdc089dab2ccc1bbd019da14098` |
| feasibility code | `scripts/audit_n1_block_feasibility.py` | `beb4c739c05a225b5fe41e796a6d7a7c0fa60239d6b14dab51f28ba6d83d75ad` |
| run 017 | `runs/2026-08-24_017_n1_block_feasibility.md` | `bf61b04a19f7586d44aec7d6f5b29b38666cce90225964c8f8af250766370eab` |
| split | `artifacts/split_regimeI.json` | `e2c065e5b395053cd655670fede8a2b117f6eb9821af884ca31c6fed3842fbab` |
| A1 admission ledger | `artifacts/a1_outer_train_admission_v1.jsonl` | `b3c1b4e11855ef4c51c5bd0c2c0009f8a24e390c511d97118c48082fc7febfd5` |

唯一 runtime scientific input 是 committed run-017 assignment metadata；feasibility YAML 只作
199-replicate parity oracle 和 summary。禁止打开 `.mat/.h5/.hdf5`、dataset root、EEG、A1
frontend/tokenizer、embedding/token/proxy cache、stimulus text、outcome 或 test identity。所有
eligible/short/cal/test EEG reads 均必须为 `0`。

### 26.4 production module 与 public API

新增：

```text
src/rc_hsg/references/__init__.py
src/rc_hsg/references/n1_joint_permutation.py
```

`__init__.py` 只导出下列 API；实现不得导出 seed、block override、K override、derangement、
邻块借用、dataset 或 value-reader 参数：
...
class N1JointPermutationSampler:
    ...
    def evaluate_real(self, values_by_row: Mapping[str, Any], select_then_score: Callable[[str, str, Any], Any]) -> N1SelectionAwareEvaluation: ...
    def evaluate_pseudo_real(self, replicate_id: int, values_by_row: Mapping[str, Any], select_then_score: Callable[[str, str, Any], Any]) -> N1SelectionAwareEvaluation: ...
```

`from_frozen_assignment` 必须 fail-closed：拒绝 symlink/path escape，验证 assignment exact hash，
解析 3,541 条 canonical rows，要求 3,481 evaluable、180 evaluable blocks、60 exclusions，且
row key、block ID、role/subject/session/length/power scope 与 run-017 ledger 自洽。canonical row
key 精确为：
...
`replicate_id` 只允许真正的 integer `1..199`，`bool` 也拒绝。unknown row、known but
non-evaluable row、缺失/多余 value keys、callback exception 均 fail closed；stable prefixes
至少为：

```text
N1_SAMPLER_INPUT_MISMATCH
N1_SAMPLER_SCOPE_MISMATCH
N1_SAMPLER_REPLICATE_INVALID
N1_SAMPLER_ROW_UNKNOWN
N1_SAMPLER_ROW_NOT_EVALUABLE
N1_SAMPLER_VALUE_SCOPE_MISMATCH
N1_SAMPLER_CALLBACK_FAILURE
N1_SAMPLER_OUTPUT_FAILURE
```

异常 detail 只允许 stable code/row key/replicate，不得包含 value、tensor、callback output 或
absolute path。

### 26.5 exact joint-permutation law 与 run-017 parity

`build(r)` 必须完全重用 §25.7 已执行的 law，禁止调用 `random`、NumPy/Torch RNG、Python
`hash()` 或重新发明 seed。对每个按 ASCII block ID 升序的 evaluable block：recipient 按
canonical row key 升序；candidate donor 按下列 tuple 升序：
...
(
  SHA256(
    ...
    + uint16_big_endian(r)
    + b"\0" + block_id_ascii
    + b"\0" + donor_row_key_ascii
  ).digest(),
  donor_row_key_ascii
)
```

按位置 zip recipient 与 sorted donor。fixed point 保留，singleton/short/edge-unavailable 不
进入 mapping，也不从邻块借 donor。每个 batch 精确 3,481 pairs、180 blocks、60 canonical
excluded keys；recipient/donor key set 相同且 scope 完全相同。

joint hash 对所有 lines 的 ASCII bytes 全局升序后连接再 SHA256：

```text
r<TAB>block_id<TAB>recipient_row_key<TAB>donor_row_key<LF>
```

对 `r=1..199`，production module 输出必须逐项匹配 feasibility YAML 已冻结的
`joint_mapping_sha256` 与 `fixed_points`；总 fixed points=`35,529`、范围=`145..214`、
199/199 hashes unique、0 bijection/cross-block violation。不得把 YAML 中的预期 hash 或
fixed-point list 硬编码进 module，也不得把 run-017 feasibility script 动态导入为 production
implementation；production module 应从 assignment 独立重建，builder 再与 YAML 对照。

### 26.6 selection-aware pseudo-real 计算边界

本轮不具备 frozen semantic schema/candidate library，因此不能计算论文 p-value；但必须把
未来统计量的对称重算边界做成可测试 API。

`values_by_row` 的 key set 必须精确等于 3,481 evaluable row keys。`evaluate_real` 按
recipient canonical order 调用同一个 `select_then_score(recipient, recipient, value)`；
`evaluate_pseudo_real(r,...)` 按相同 recipient order 调用
`select_then_score(recipient, donor_r, values_by_row[donor_r])`。二者均恰调用 3,481 次，
返回顺序固定，60 exclusions 相同；pseudo-real 的 `joint_mapping_sha256` 必须等于
`build(r)`，real 为 `None`。

callback 必须自己完成“从 frozen candidate library 选择 candidate/parent-consistent path，
再计算该 source 下 score”的完整操作。module 不缓存 candidate、real-selected winner、path
或 score，不提供 candidate-specific shortcut，也不计算

```text
p = (1 + sum_r I[T(E^(r)) >= T(E)])) / 200
```

只有以后 schema/candidate/path 合同冻结并通过 Gate R0 后，才可在 outer-train mechanism
analysis 中使用同一 callback 对 real 与每个 pseudo-real 重算
`T_l(E;C)=max_y s_l(E,y)`。不得把本轮 synthetic callback tests 写成 exchangeability、
semantic validity 或 randomization-p PASS evidence。
...
path 为 `scripts/build_n1_sampler_contract.py`。production CLI 只接受
`--output-root --verification-root-a --verification-root-b`；verification roots 必须
repository-external。不得接受 dataset、seed、K、role、block、threshold、resume、candidate、
score、device 或 hash override。builder 必须从 production module 生成 199 batches，与
feasibility YAML 逐项比较后一次性在内存 render 三个 outputs，再原子写入 verification A、
verification B、canonical root，并逐文件 byte-compare；不得复制 canonical bytes 伪装成三次
build，也不得写 recipient/donor relations。

canonical scientific outputs 精确为：

```text
artifacts/nulls/n1_contract.yaml
artifacts/nulls/n1_permutation_manifest_v1.jsonl
reports/n1_selfcheck.md
```
...
其余 top-level order 固定为：

```text
input_artifacts, assignment_scope, permutation_contract, parity,
selection_aware_boundary, implementation, outputs, safety, downstream_boundary
```

manifest 恰有 199 JSONL rows，按 replicate ID 升序，每 line key order 精确为：
...
`fixed_point_rate` 用 `format(fixed_points / 3481, ".12f")` 的 JSON string，JSON 采用 UTF-8、
ASCII、compact separators 和 LF。manifest 不含 block/recipient/donor key 或 relation。report
只汇总 input hashes、3,541/3,481/180/60、199 parity、35,529 fixed points、no-read/no-score
边界、DEGRADED/mechanism-only consequence 与 next=N2。

### 26.8 tests 与 run-018 acceptance

新增：

```text
tests/test_n1_joint_permutation.py
tests/test_build_n1_sampler_contract.py
```

至少覆盖：assignment hash/schema/count/key/block/scope；symlink/path escape；replicate
`0/200/bool/non-int`；unknown 与 non-evaluable row；每 replicate 3,481-row/180-block bijection；
fixed points 保留；199 exact hash/fixed-point parity；199 unique；35,529 total；deterministic
rebuild；tampered assignment/role/block/status/YAML parity rejection；无 random/Python hash/RNG；
manifest 199 rows 与 exact key order；artifact 无 recipient/donor relation；atomic failure；
unknown CLI；three-root byte identity；`.mat/.h5/.hdf5`/h5py/frontend/A1/EEG reader 禁止。

selection-aware synthetic tests 必须证明：real/pseudo-real 调用同一个 callable；各自恰
3,481 calls 且 order 固定；donor source value 变化会使 synthetic winner 改变，确认未复用
real-selected candidate；一个 L1→L2→L3 parent-path fixture 对每个 pseudo source 完整重选
parent-consistent path；missing/extra value、callback failure fail closed；API 中不存在
candidate-specific p-value shortcut。synthetic tests 不能读取真实 EEG、语义 outcome 或 test。

服务器最低命令：

```text
.venv/bin/python -m unittest discover -s tests -p 'test_n1_joint_permutation.py'
.venv/bin/python -m unittest discover -s tests -p 'test_build_n1_sampler_contract.py'
.venv/bin/python -m unittest discover -s tests -p 'test_audit_n1_block_feasibility.py'
.venv/bin/python -m unittest discover -s tests -p 'test_project_memory.py'
.venv/bin/python -m unittest discover -s tests
.venv/bin/python scripts/check_project_state.py
...
全部必须实际 PASS/no skip，记录实际 test 数。full discovery 是 regression authority；禁止
运行 feasibility production CLI 或任何会打开真实 dataset 的 command。审 diff 并证明没有
EEG/value/token/embedding/proxy、candidate/score/p-value、recipient-donor relation、text、
outcome、cache、large binary 或历史文件变化。

### 26.9 状态迁移与 run record

导入后新增 `SPEC_V27_REVIEW`，prerequisite=`[SPEC_V26_REVIEW]`，status=`DONE`，evidence 为
本 SPEC/review，completion provenance 为 run 018。`S0_N1_SAMPLER.produces` 更新为 §26.4、
§26.7、tests 和 `runs/2026-08-24_018_n1_mechanism_sampler.md`；完成 evidence scope 使用
§26.7 exact string。

仅在所有 acceptance PASS 后机械迁移：

```text
SPEC_V27_REVIEW       DONE
S0_N1_SAMPLER         DONE
S0_N2_SAMPLER         READY; owner=CHATGPT_OR_AUTHOR
task counts           74 total / 41 DONE / 8 SKIPPED / 24 BLOCKED / 1 READY
last_completed_task   S0_N1_SAMPLER
recommended_next_task S0_N2_SAMPLER
last_run              runs/2026-08-24_018_n1_mechanism_sampler.md
repository_status     RC_HSG_V27_N1_MECHANISM_SAMPLER_IMPLEMENTED_N2_PENDING
execution             stage_0 / READY
route.locked          null
test_status            LOCKED_UNTIL_ROUTE_LOCK
```
...
```text
project.spec_version      v2.7
project.spec_path         guide/RC_HSG_Paper_Spec_v2_7_2026-08-24.md
project.baseline_commit   082ed4f72f1b8bbc18096a5f0caea2075b2783c4
project.reviewed_commit   082ed4f72f1b8bbc18096a5f0caea2075b2783c4
```

B_V4 保持 active，reason/resolution 更新为：N1 mechanism-only index sampler 已实现，但因
run-017 population coverage 降级而不能成为 primary；N2 common-phase construction、
admissibility tolerances 与 outcome-blind Gate R0 仍未冻结/实现。`GATE_R0` 继续 BLOCKED；
route 继续 unlocked；test 继续 locked。同步 `AGENTS.md`、`AI_START_HERE.md`、`HANDOFF.md`、
repository-root `CODEX_NEXT_TASK.md`、`TASKS.yaml`、`PROJECT_STATE.yaml`、implementation matrix、
snapshots、validator/status/tests。旧 SPEC/review/run、run-017 script/ledger/YAML/report、split、
A code 与历史 artifacts byte-immutable。

run 018 必须记录：import/code/output hashes、实际 tests、199 parity、counts/fixed-point summary、
zero EEG/text/outcome/test reads、selection-aware synthetic boundary、state counts 和下一 owner。
不得声称 N1 primary admission、semantic selection/p-value validity、Gate R0 PASS 或 N2 已研究。

### 26.10 论文下一步与 run-018 硬停止线

论文当前已完成“可审计输入/前端 + early leakage + N1 结构可行性”，run 018 只把 N1 的
索引随机化律变成可复用、可测试代码。下一真正科学研究是 `S0_N2_SAMPLER`：ChatGPT/author
必须先冻结 multivariate common-phase 变换的 FFT/endpoint/mask/length handling、共享相位
范围、real/surrogate nuisance diagnostics、PSD/cross-spectrum/covariance tolerances、coverage、
replicates、admissibility decision 与 fail route；Codex 不得自主选择这些参数。

run 018 成功后 commit/push `main`，fetch 后要求 local HEAD=`origin/main`、clean，validator/
status post-push PASS，然后在 sole READY `S0_N2_SAMPLER` 处硬停止。禁止开始 N2、Gate R0、
F/schema/candidates/reference features/reliability/calibration/training/test；禁止读取任何真实
array 或产生 donor value。任何需要改变 run-017 assignment、hash-sort law、fixed-point policy、
selection-aware symmetry、N1 claim consequence 或 next-task route 的情况，停止回传
ChatGPT/author，不得自主裁决。

---

## 27. v2.8：synthetic-only N2 multivariate common-phase sampler 精确合同

### 27.1 远程复核与 run-018 接受结论

本版本复核的 clean remote baseline 固定为：

```text
branch                         main
HEAD = origin/main             06e3e5f9b5c720bbb29074ca1cae1109add5b1b9
commit                         Implement RC-HSG v2.7 N1 mechanism sampler
tasks                          74 total / 41 DONE / 8 SKIPPED / 24 BLOCKED / 1 READY
sole READY                     S0_N2_SAMPLER
owner                          CHATGPT_OR_AUTHOR
route.locked                   null
test_status                    LOCKED_UNTIL_ROUTE_LOCK
B_V4                           active
...
validator、status、`git diff --check` 均通过。本地独立环境实际复现 N1 新 suites 合计
`18/18` 与 project-memory `61/61`；服务器 run 018 记录 full discovery `239/239`、no skip。
本地 full discovery 因环境缺 `torch/h5py` 出现 6 个 import errors，故不冒充复现服务器 full
suite。N1 production module/contract/manifest/report hashes 与 run 018 一致，manifest 恰 199
rows 且没有 recipient/donor/block relation；实现由 assignment 动态重建，不含 RNG、Python
`hash()`、frontend 或 dataset reader。run 018 科学结果与状态迁移接受。

### 27.2 run-018 provenance typo 的追加更正

run 018 不可改写文本把 package-source `CODEX_NEXT_TASK.md` SHA256 写成：

```text
667b36d04a5e91fd314bf44b1e7ce0a145ed0e9a45286c36c56c8eb8c9d2b0e7
```

但其同时记录的 ZIP SHA256
`934d7bb625b6a5183d251ae0d7b5255053adaebef17a0883394a371f3f5b5c24` 对应 ZIP 内该文件及
`PACKAGE_MANIFEST.sha256` 的实际值均为：

```text
667b8bc2af414673e09d9d2011446db502fbca305fb26e6c558bd0a762d51ef6
```

这是一个单字段 transcription/provenance typo，不影响导入的 v2.7 SPEC/review、N1 code/
outputs/tests 或 state。run 019 必须新增
`artifacts/governance/run018_provenance_correction.yaml`，记录 historical run path/hash、ZIP
hash、recorded/corrected value、verification basis、`scientific_state_changed=false`；不得修改
run 018 或旧 package。

### 27.3 N2 的科学依据与不可升级边界

[Prichard 与 Theiler 的 multivariate Fourier surrogate](https://arxiv.org/abs/comp-gas/9405002)
给出的关键构造是：在同一频率对所有同步通道施加同一个随机相位增量，从而保持每通道
periodogram 以及通道间相位差/线性交叉谱。该论文也明确指出，Fourier 方法精确保留的是
circular auto/cross-correlation，而不是有限非周期序列的一般线性结构。

[Schreiber 与 Schmitz 的综述](https://arxiv.org/abs/chao-dyn/9909037) 强调 Fourier
surrogate 的 endpoint mismatch 可能制造可辨识的高频伪迹；
[Theiler 等人的 surrogate-data framework](https://doi.org/10.1016/0167-2789(92)90102-S)
要求先明确 null hypothesis 与保持量，再用独立统计量比较 original 与 surrogate。因此：

1. N2 是 **empirical matched reference**，不是 exact knockoff、随机化检验、FDR 或
   distribution-free null；
2. synthetic preservation PASS 只证明算法实现正确，不证明真实 EEG 难以区分；
3. amplitude distribution、endpoint jump/slip 与 real/reference classifier 可能揭示伪迹，
   必须在下一轮 Gate R0 用真实但 outcome-blind 数据决定；
4. AAFT/IAAFT、逐通道独立相位、windowing、padding、truncation 或先平移端点均不是本轮
   primary，不得加入候选 grid 或事后替换 common-phase；
5. N1 保持 mechanism/robustness only；N2 只有 Gate R0-N2 PASS 后才成为 primary。

### 27.4 exact transform law

policy ID：

```text
RC_HSG_N2_MULTIVARIATE_COMMON_PHASE_FOURIER_V1
```

对 unpadded、finite、CPU `float32` trial
`x ∈ R^(105×T)`，`T>=500`，先以 `float64` 计算
`X=rfft(x,n=T,axis=-1,norm=backward)`。replicate `r∈{1,...,199}` 与 canonical ASCII
row key 的 seed digest：

```text
SHA256(
  b"RC_HSG_N2_COMMON_PHASE_V1\0SEED\0"
  + uint16_big_endian(r)
  + b"\0"
  + row_key_ascii
)
...
取 digest 前 16 bytes 的 big-endian integer 作为 NumPy `PCG64` seed；必须使用
`numpy==2.5.2` 的 local `numpy.random.Generator(PCG64(seed))`，不读写 global NumPy/Torch/
Python RNG state。对 positive interior bins 一次生成 `float64 U[-π,π)` angles。multiplier：

```text
m[0] = 1+0j
if T is even: m[T/2] = 1+0j
all positive interior k: m[k] = cos(theta[k]) + i*sin(theta[k])
```

同一 `m[k]` 广播到全部 105 channels；`y64=irfft(X*m,n=T,norm=backward)`，验证 finite 后
cast 为 contiguous CPU `float32`。不得中心化、标准化、clamp、window、zero-pad、truncate、
circular-shift、endpoint repair、amplitude remap、channel mixing 或独立逐通道抽 phase。
fixed/near-zero phases 不拒绝、不重抽。`rfft/irfft` 隐式构造负频率共轭；DC 与偶数长度
Nyquist 不旋转，确保 real output 与 mean preservation。
...
production 只能使用冻结 split/eligibility ledger 中的 row key；module 层只验证 ASCII 格式，
caller/后续 Gate R0 负责 exact key authorization。

### 27.5 public API、padding 与 fail-closed

新增 `src/rc_hsg/references/n2_common_phase.py`，并由现有 references package 追加导出：
...
class N2CommonPhaseSampler:
    def generate_unpadded(self, values: torch.Tensor, row_key: str, replicate_id: int) -> N2TrialSurrogate: ...
    def generate_padded(self, values: torch.Tensor, valid_samples: torch.Tensor, row_keys: tuple[str, ...], replicate_id: int) -> N2BatchSurrogate: ...
```

unpadded input 必须为 CPU、contiguous、`torch.float32`、rank 2、shape `[105,T]`、
`T>=500`、finite、`requires_grad=false`。padded input 必须为 CPU contiguous float32 rank 3
`[B,105,Tmax]`；`valid_samples` 必须 CPU int64 `[B]`，每项 `500<=T_i<=Tmax`；row keys 数量
与 B 相同、unique、canonical。只允许读取每 row 的 `values[i,:,:T_i]`；padding tail 不参与
FFT，output tail 强制 exact zero，返回 mask 精确 true-prefix/false-tail。input tensor 不得原位
修改，output shape/dtype/device/length/order 与输入合同一致。

`replicate_id` 只允许真正的 integer 1..199，bool 拒绝。错误 detail 只含 stable check、row
key/replicate，不含 tensor value、phase、absolute path。stable prefixes 至少：

```text
N2_COMMON_PHASE_INPUT_MISMATCH
N2_COMMON_PHASE_ROW_KEY_INVALID
N2_COMMON_PHASE_REPLICATE_INVALID
N2_COMMON_PHASE_NUMERIC_FAILURE
N2_COMMON_PHASE_MASK_FAILURE
N2_COMMON_PHASE_OUTPUT_FAILURE
```

module 不得 import h5py、A1/frontend、split loader、dataset reader 或项目 script；不得打开
文件、保存 waveform/phase/value、调用 full encoder、计算 score/p-value 或训练。

### 27.6 synthetic preservation diagnostics

所有 preservation metric 在 original float32 input `x` 与 returned float32 output `y` 上以
float64 重算。只用稳定的 global relative norms，不用 near-zero per-bin relative error：

```text
R_psd  = || |rfft(y)|^2 - |rfft(x)|^2 ||_F / max(|| |rfft(x)|^2 ||_F, 1e-12)
R_cov  = || Cov(y)-Cov(x) ||_F / max(||Cov(x)||_F, 1e-12)
R_mean = || mean_t(y)-mean_t(x) ||_2 / max(||mean_t(x)||_2, rms(x), 1e-12)
R_xspec(P) = || conj(Y_j)Y_k-conj(X_j)X_k ||_F /
             max(||conj(X_j)X_k||_F,1e-12), aggregated over pair set P
```

synthetic acceptance：`R_psd,R_cov,R_mean,R_xspec <= 1e-6`；all finite；input unchanged；
even/odd DC/Nyquist/conjugate behavior PASS；199 seed hashes unique；same row/r returns bitwise-
identical output on same locked CPU environment；different replicate output 不同；batch/unpadded
prefix identical；mask/zero-tail exact。

cross-spectrum test panel：T=500 fixture 对全部 `105×105` channel pairs；其余 lengths 对固定
pairs `(0,1),(0,52),(0,104),(1,2),(26,78),(52,104),(103,104)`。deterministic analytic fixtures
覆盖 `T∈{500,501,513,2048,27010}` 与 replicates `{1,2,199}`；另对 T=513 的同一 row 生成
全部 199 replicates，验证 finite、unique seed 与 deterministic replay。fixtures 由 sin/cos、
channel mixing、deterministic impulse/saw components 解析生成，不调用 RNG，不含真实 EEG。

同时只记录而不设 synthetic Gate cutoff：

```text
gamma_jump = sum_c (x[c,0]-x[c,T-1])^2 /
             max(sum_c,t (x[c,t]-mean_t(x[c]))^2, 1e-12)
gamma_slip = sum_c ((x[c,1]-x[c,0])-(x[c,T-1]-x[c,T-2]))^2 /
             max(sum_c,t>0 (x[c,t]-x[c,t-1])^2, 1e-12)
```

以及 per-channel amplitude two-sample KS、quantile shift、real-surrogate waveform correlation。
这些量不应被算法强制相等；它们为 Gate R0 的 endpoint/amplitude artifact contract 提供 schema，
不得因 synthetic 数值好看而预判真实 N2 PASS。

### 27.7 builder、产物与 provenance correction

新增：

```text
scripts/build_n2_sampler_contract.py
tests/test_n2_common_phase.py
tests/test_build_n2_sampler_contract.py
```

builder API：

```python
build_n2_sampler_contract(
    project_root: Path,
    canonical_output_root: Path,
    verification_output_roots: tuple[Path, Path],
    *,
...
production CLI 只允许 `--output-root --verification-root-a --verification-root-b`；禁止 dataset、
input、role、seed、K、FFT、dtype、device、tolerance、fixture、resume 或 hash override。验证
§27.8 fixed inputs 后，运行 §27.6 synthetic grid；全部 bytes 在内存 render，分别 build/write
verification A、B 和 canonical，原子替换并 byte-compare。

canonical outputs：

```text
artifacts/governance/run018_provenance_correction.yaml
artifacts/nulls/n2_contract.yaml
reports/n2_selfcheck.md
```
...
```text
schema_version: 1
artifact: RC_HSG_N2_MULTIVARIATE_COMMON_PHASE_FOURIER_V1
spec_version: v2.8
...
```text
input_artifacts, scientific_basis, transform_contract, seed_contract,
input_output_contract, synthetic_fixtures, preservation_thresholds,
synthetic_diagnostics, artifact_diagnostics_schema, implementation,
prohibited, safety, downstream_boundary
```

所有 diagnostic float 以 `float.hex()` string 保存；不得保存 fixture waveform、FFT、phase/
seed integer、channel matrix 或 per-sample value。report 必须明确 synthetic-only、common phase
保持 circular second-order structure、amplitude/endpoint 未保证、N2 尚未 admitted、下一任务
Gate R0。

### 27.8 fixed inputs 与 tests

run 019 fixed inputs：

| label | path | SHA256 |
|---|---|---|
| v2.7 SPEC | `guide/RC_HSG_Paper_Spec_v2_7_2026-08-24.md` | `80d613bcb1eb5e3d3948f71f225ffcab5be52c6593fb141fdf410eb0bd753951` |
| v2.7 review | `artifacts/spec_review/rc_hsg_v27_n1_mechanism_sampler_review.md` | `bd245a03d4244f18381b1008ddbd0504cf7ea28f19407cb254747c20150894eb` |
| run 018 | `runs/2026-08-24_018_n1_mechanism_sampler.md` | `0b3f9ee0662f3429b3ac6fe0b78148e5b61aa2de21d59bec97f8cc634b90d4e7` |
| N1 module | `src/rc_hsg/references/n1_joint_permutation.py` | `888c6965c89c007e7edb4d0bcf513a8cdcaf4201dff6b05a3f7bf75bf7a94ca6` |
| N1 contract | `artifacts/nulls/n1_contract.yaml` | `4fee63f743936db06eea41164f85f67228785872d3fca2098e657b1dc0383729` |
| N1 manifest | `artifacts/nulls/n1_permutation_manifest_v1.jsonl` | `b7e68368799be446af60dcec029458e4e769f6605c1c56c032b76fb069f38c06` |
| A eligibility | `artifacts/a_interface_eligibility_v1.jsonl` | `8eded8fb2786747e96b8388d4d91315e39db9f8a9eb25ea69056d219e1e8e1ad` |
| requirements | `requirements-trust-align.lock.txt` | `72a2a3274ef9516dba95a4f4022cacfba0e02d10445e1618da2a569f59381910` |

tests 至少覆盖：rank/channel/dtype/device/contiguous/requires-grad/finite/min-length；canonical
row key；replicate 0/200/bool/non-int；even/odd DC/Nyquist/conjugate symmetry；same phase across
channels；synthetic full/pair cross-spectrum；PSD/cov/mean thresholds；input immutability；same
replicate replay；different replicate；199 seed uniqueness/finite；local RNG isolation；padded
prefix-only transform、arbitrary/nonfinite tail ignored、mask、zero tail、batch-unpadded equality；
independent-channel-phase negative mutation；DC/Nyquist rotation negative mutation；padding-before-
FFT negative mutation；amplitude/endpoint schema finite；no waveform/phase/value output；builder
fixed hashes、triple-render、atomic failure、symlink/path escape、unknown CLI；AST 禁止 h5py、
dataset/A1/frontend/importlib/project-script I/O 与 score/p-value/training。

服务器最低验证：

```text
.venv/bin/python -m unittest discover -s tests -p 'test_n2_common_phase.py'
.venv/bin/python -m unittest discover -s tests -p 'test_build_n2_sampler_contract.py'
.venv/bin/python -m unittest discover -s tests -p 'test_n1_joint_permutation.py'
.venv/bin/python -m unittest discover -s tests -p 'test_build_n1_sampler_contract.py'
.venv/bin/python -m unittest discover -s tests -p 'test_native_spectral_a1.py'
.venv/bin/python -m unittest discover -s tests -p 'test_project_memory.py'
...
全部必须实际 PASS/no skip，记录实际数。禁止运行任何真实 dataset CLI。审 diff 不得含真实
EEG、fixture waveform/FFT/phase、text/outcome/test、reference score/p-value、cache 或 large
binary；旧 SPEC/review/run/N1/A artifacts byte-immutable。

### 27.9 状态迁移

导入后新增 `SPEC_V28_REVIEW`，prerequisite=`[SPEC_V27_REVIEW]`，status DONE；完成
`S0_N2_SAMPLER` 时 produces/evidence 更新为 §27.5/§27.7/test/run-019，evidence scope 使用
§27.7 exact string。仅在全部 acceptance PASS 后：

```text
SPEC_V28_REVIEW       DONE
S0_N2_SAMPLER         DONE
GATE_R0               READY; owner=CHATGPT_OR_AUTHOR
task counts           75 total / 43 DONE / 8 SKIPPED / 23 BLOCKED / 1 READY
last_completed_task   S0_N2_SAMPLER
recommended_next_task GATE_R0
last_run              runs/2026-08-24_019_n2_common_phase_sampler.md
repository_status     RC_HSG_V28_N2_COMMON_PHASE_SAMPLER_IMPLEMENTED_GATE_R0_AUDIT_PENDING
execution             gate_r0 / READY
route.locked          null
test_status            LOCKED_UNTIL_ROUTE_LOCK
```
...
```text
project.spec_version      v2.8
project.spec_path         guide/RC_HSG_Paper_Spec_v2_8_2026-08-24.md
project.baseline_commit   06e3e5f9b5c720bbb29074ca1cae1109add5b1b9
project.reviewed_commit   06e3e5f9b5c720bbb29074ca1cae1109add5b1b9
```

B_V4 保持 active，reason 改为：N1 mechanism sampler 与 N2 synthetic sampler 已实现，但
N1 仍非 primary，N2 的真实 outcome-blind admissibility/Gate R0 未执行。`GATE_R0` 移除
blocked reason，成为 sole READY，why_ready 明确只允许下一版 author-frozen real audit。
同步 AGENTS、AI_START_HERE、HANDOFF、CODEX_NEXT_TASK、TASKS、PROJECT_STATE、implementation
matrix、snapshots、validator/status/tests；所有 Gate outcome 仍 null，route unlocked，test locked。

### 27.10 run-019 硬停止线与论文下一步

run 019 只授权 code/synthetic diagnostics/provenance correction，真实数组读取总数必须为 0。
成功后 commit/push `main`，fetch，要求 HEAD=origin/main、clean、post-push validator/status PASS，
然后 STOP。不得执行 Gate R0、打开 outer-train/cal/test EEG、读取文本/outcome、加载 A1、
生成 embeddings/reference scores、训练、锁 route 或解锁 test。

下一轮 ChatGPT/author 必须另行冻结 Gate R0 的真实 outcome-blind audit：legal role/read scope、
bounded/full replicate plan、balanced real/reference classifier、subject/session/length/power probes、
PSD/cross-spectrum/covariance numerical audit、amplitude/endpoint/mask diagnostics、coverage、
预注册 tolerances 与 family failure route。只有该 Gate PASS 才允许 N2 成为论文 primary
reference；本轮 synthetic PASS 不得提前改变摘要、标题或主张。

---

## 28. v2.9：Gate R0 outcome-blind real-reference integrity 精确合同

### 28.1 远程复核、run-019 接受与非阻断清洁度记录

本版本复核的 clean remote baseline 固定为：

```text
branch                         main
HEAD = origin/main             4fa6fadc8bdee0d163acc8bf9ee48aeac4d3095d
parent implementation commit   f8dce4168ac123c51b5cb1db474734f83bd60799
baseline parent                06e3e5f9b5c720bbb29074ca1cae1109add5b1b9
HEAD commit                     Correct run 019 repository status
tasks                           75 total / 43 DONE / 8 SKIPPED / 23 BLOCKED / 1 READY
sole READY                      GATE_R0
owner                           CHATGPT_OR_AUTHOR
route.locked                    null
test_status                     LOCKED_UNTIL_ROUTE_LOCK
...
`scripts/check_project_state.py`、`scripts/project_status.py`、`git diff --check` 与 clean remote
复核均 PASS。run-019 记录 N2 suites `17/17+10/10`、N1 regressions `9/9+9/9`、native A1
`12/12`、project-memory `63/63`、full discovery `268/268`、0 skipped。committed N2 hashes与
run 一致；15 个 synthetic grid cases 的最大 PSD/covariance/mean/cross-spectrum relative norm
分别不超过 `3.64e-9/2.54e-9/5.53e-9/4.47e-9`，199 seed/output fingerprints unique，真实
EEG/text/outcome/test reads 为零。run-019 N2 sampler evidence 接受，但仍不等于 real admission。

第二个 commit 只把 run-019 的 repository status 从
`...GATE_R0_PENDING` 改为 state 中的 exact `...GATE_R0_AUDIT_PENDING`。Git history 保留旧
blob，但直接修改已提交 run file 不符合最严格 append-only 习惯；run-020 必须新增
`artifacts/governance/run019_postcommit_correction.yaml`，绑定两个 commit、旧/新 run hashes、
字段和值，并声明 `scientific_state_changed=false`，不得再修改 run-019。另记录 N2 builder 的
`_diagnostics` source literal 含两个完全相同的 `covariance_relative_norm` key；Python runtime
只保留后一相同值，committed artifact/测试/科学量不变。为保持 run-019 code/artifact hash
provenance，本轮不修改该 historical builder；后续新脚本不得复制重复键。

correction artifact 的 exact provenance：
...
corrected_commit            4fa6fadc8bdee0d163acc8bf9ee48aeac4d3095d
corrected_run_sha256        764778bae4f873e66e806beced6c0c7c336c750b7c30b6a510c3e6fe02526996
corrected_repository_status RC_HSG_V28_N2_COMMON_PHASE_SAMPLER_IMPLEMENTED_GATE_R0_AUDIT_PENDING
scientific_state_changed    false
```

### 28.2 Gate 的唯一问题、合法 population 与数据防火墙

Gate R0 只回答：N1/N2 是否具有足够完整、不可由冻结 nuisance/artifact 轻易识别的 reference
integrity，可否进入后续 reliability pipeline。它不回答 semantic decoding、performance、
calibration、brain attribution 或机制问题。

唯一合法真实值 population 为 Regime-I outer-train：

| role | all rows | A1 eligible / read exactly once | short / no read |
|---|---:|---:|---:|
| train_fit | 2,832 | 2,797 | 35 |
| inner_val | 709 | 700 | 9 |
| total | 3,541 | 3,497 | 44 |

18 subjects、18 admitted summary files、session=1、task=NR；eligible valid length `513..18436`。
`train_fit` 只用于拟合 audit standardizer/classifier/probes，`inner_val` 只用于 Gate 判定。
不得交换、合并或从 inner-val 调参。cal/cal-select/cal-cert/test arrays、44 short arrays、stimulus
text、semantic label/outcome、prediction、historical metric 与 test identity 的 read count 必须为 0。
不得使用 Regime II 或解锁 test。

生产 scan 只能经 `scripts/validate_a1_frontend.py` 已审计的 `_dataset_files/_read_raw`，source
allowlist 精确等于 frozen admission ledger 的 3,497 eligible keys；ASCII row key 为
`subject+TAB+slot_6_digits+TAB+occurrence_id`。按 `(source_file ASCII, slot, subject,
occurrence_id)` 排序，每个 dataset array 精确 dereference 一次；同一次内存驻留中完成 real
features、三个 N2 replicates、diagnostics，再释放 raw/surrogate。禁止 waveform/cache/token/
FFT/phase/model serialization；不得为 replay 二次读取 dataset。

N2 eligible-conditional coverage 必须 `3497/3497=1.0`；包含既定 forced-L0 的 system population
coverage 只作事实报告 `3497/3541=0.9875741316012426`，44 short 不被伪装成 N2 failure。
任一 eligible row 无法产生全部审计 replicates 则 N2 coverage FAIL。

### 28.3 fixed inputs

第一次真实 dereference 前必须验证：

| label | path | SHA256 |
|---|---|---|
| v2.8 SPEC | `guide/RC_HSG_Paper_Spec_v2_8_2026-08-24.md` | `f718fc37875a6dac7c539260de054d9f9c52966905b1912cf193d573a0424f23` |
| v2.8 review | `artifacts/spec_review/rc_hsg_v28_n2_common_phase_sampler_review.md` | `66edb1aca13e01f87d1a162b86254bbad87ce207ae208474f46a326e53948ea7` |
| run-019 corrected | `runs/2026-08-24_019_n2_common_phase_sampler.md` | `764778bae4f873e66e806beced6c0c7c336c750b7c30b6a510c3e6fe02526996` |
| N2 module | `src/rc_hsg/references/n2_common_phase.py` | `65fc0c3215a2b289c498e989795db74002642388ca64caa2fea93d7780a5aa7e` |
| N2 builder | `scripts/build_n2_sampler_contract.py` | `baebfa04bf2381075786d9375e78a741ded32f157ea21da37885bc4001530252` |
| N2 contract | `artifacts/nulls/n2_contract.yaml` | `c2713dc4fbe989c1680e02e88c336541482bfcb9e828170b3a225d2466d1377d` |
| N2 report | `reports/n2_selfcheck.md` | `042fc06f0627d4b29ead30075bb003800b0305ceb7d67387bc3f3e9d2f15f13c` |
| eligibility | `artifacts/a_interface_eligibility_v1.jsonl` | `8eded8fb2786747e96b8388d4d91315e39db9f8a9eb25ea69056d219e1e8e1ad` |
| admission ledger | `artifacts/a1_outer_train_admission_v1.jsonl` | `b3c1b4e11855ef4c51c5bd0c2c0009f8a24e390c511d97118c48082fc7febfd5` |
| admission freeze | `artifacts/a1_outer_train_admission_freeze.yaml` | `e973fbbe841a47f027cbf0f8a8ad65e66d106d675e8ed838dd0daf4a08dcab12` |
| N1 assignment | `artifacts/nulls/n1_block_assignment_v1.jsonl` | `d0acc5e5fe78bc36a69cb04b6f605983c675e49a764538ae1665f86a28acee04` |
| N1 feasibility | `artifacts/nulls/n1_block_feasibility.yaml` | `90a6178100f507299e12223d15291699aad84e4b58bb52e29843dbf99ee6f771` |
| N1 contract | `artifacts/nulls/n1_contract.yaml` | `4fee63f743936db06eea41164f85f67228785872d3fca2098e657b1dc0383729` |
| N1 manifest | `artifacts/nulls/n1_permutation_manifest_v1.jsonl` | `b7e68368799be446af60dcec029458e4e769f6605c1c56c032b76fb069f38c06` |
| Regime-I split | `artifacts/split_regimeI.json` | `e2c065e5b395053cd655670fede8a2b117f6eb9821af884ca31c6fed3842fbab` |
| subject population | `artifacts/gate_a_population.yaml` | `279e3edf1c41971b6967f74657ec531533977d90ea4dc3d48a5efd63dd295d60` |
| A1 code | `src/rc_hsg/backbones/native_spectral_a1.py` | `71ae12d65cc0acc6fd5870434e141ee7d849eb8befa718a84fb99cb86ed533d9` |
| audited reader | `scripts/validate_a1_frontend.py` | `ecc84a0363629e919409321cdc73327b6e3c7e779e224a18ab55a6b6ac6777cd` |
| requirements | `requirements-trust-align.lock.txt` | `72a2a3274ef9516dba95a4f4022cacfba0e02d10445e1618da2a569f59381910` |

requirements 必须精确含 `numpy==2.5.2`、`scikit-learn==1.9.0`、`scipy==1.18.0`、
`torch==2.13.0`。任一 fixed hash、count、role、subject、file identity、dtype/shape、split/test
lock 不一致，`STATE_SPEC_CONFLICT`，在真实 read 前停止。

### 28.4 replicate schedule 与 outcome-blind numerical panel

所有 3,497 eligible rows 在一次 read 中生成 frozen replicates `(1,2,199)`，共 10,491
surrogates；每个必须 finite、shape/length/dtype 一致、input unchanged、phase digest unique、A1
window count/mask 与 real 一致。对 deterministic audit panel 还立即重放 replicate 1 并要求
bitwise equality，不二次读 source。

panel law：只取 `power_edge_status=PASS` rows；对全部
`subject(18) × role(2) × length_bin(3) × power_bin(2)` 216 cells，以

```text
SHA256(b"RC_HSG_GATE_R0_V1\0PANEL\0" + row_key_ascii)
```

升序选每 cell 第一 row，必须恰 216 unique rows。4 个 power-edge-unavailable eligible rows仍在
full coverage/classifier/amplitude/endpoint audit，但不进入 power probe/panel。

真实数值 audit：

- replicate 1：全部 3,497 rows；
- replicates 2/199：216-row panel；
- metric 以 original/returned float32、float64 重算，使用 §27.6 的 `R_psd/R_cov/R_mean`；
- `R_xspec` 固定 pairs `(0,1),(0,52),(0,104),(1,2),(26,78),(52,104),(103,104)`；
- 每 row/replicate 的四项都必须 finite 且 `<=1e-6`；不得只检查 aggregate maximum；
- report 保存 count、subject/role strata 与 maximum/median/quantiles 的 `float.hex()`，不保存
  per-row metric/value。

common-phase 保持 circular second-order structure 的依据仍是
[Prichard 与 Theiler](https://arxiv.org/abs/comp-gas/9405002)；endpoint/nonstationarity 风险依据
[Schreiber 与 Schmitz](https://arxiv.org/abs/chao-dyn/9909037)。该数学保持不替代下述真实
artifact/classifier Gate。

### 28.5 A1-path audit feature、real/N2 classifier 与 nuisance transfer

real 与每个 surrogate 必须调用同一 frozen `NativeSpectralA1._spectral_tokens`，禁止 full
projection/Transformer encoder、training/backward/dropout。对每 row 构造唯一 audit vector
`g(x)`，所有计算 float64、字段顺序固定：

1. 840 token dimensions 各自 across-window mean，再接 population SD：1,680 features；
2. 按 A1 同一规则逐 channel median-center，scale=`max(1.4826*MAD,RMS,1e-6)`、clip
   `[-20,20]`；每 channel 的 q=`.01,.10,.25,.50,.75,.90,.99` quantiles，channel-major：735；
3. normalized signal 每 channel 的 absolute endpoint jump 与 first-difference slip；分别取上述
   7 个 across-channel quantiles：14。

总维度精确 2,429。不得加入 row/subject/session/role/length/power label、raw amplitude、text、
outcome、FFT phase、candidate 或 score。非 finite feature 立即 technical abort。

对每个 replicate 独立训练一个 balanced real/N2 binary audit dataset：每 train-fit row 恰一 real
和一 surrogate，label `real=0,surrogate=1`；inner-val 同样 paired，只评估不拟合。standardizer
只在该 replicate 的 train-fit combined pairs 上 fit：float64 mean/population SD，SD `<1e-12`
的 column 删除，其余 z-score。classifier 精确为 scikit-learn 1.9.0
`LogisticRegression(penalty="l2",C=1.0,solver="lbfgs",tol=1e-8,max_iter=2000,
fit_intercept=True,class_weight=None,warm_start=False)`；不得 CV/grid/early-stop/class-weight。
`n_iter_>=2000`、warning、fit error 或 replay mismatch 为 technical abort，不得换 solver/C。
[scikit-learn 官方文档](https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.LogisticRegression.html)
确认 lbfgs/L2 与 dense float64 contract。

inner-val 每 subject 都有 paired two classes。对每 subject 计算 ROC AUC，18 subjects 等权平均；
用 `artifacts/gate_a_population.yaml` 精确重建并验证 SHA256=`e77ca92b29c414a17c1e66edf4075edd470c007dfe2019487c36758f5f99c86d`
的 10,000×18 bootstrap indices。upper bound 为排序后 index 9,749（zero-based）的 conservative
97.5% order statistic。每 replicate 同时要求 subject-macro point AUC `<=0.65` 且 upper
`<=0.65`；pooled AUC/AUPRC/Brier 只报告。point≤0.65 但 upper>0.65 为
`NOT_ADMITTED_INCONCLUSIVE`，不是 PASS。

nuisance probes 使用同一 `g(x)`，但 standardizer/model 只在 **real train-fit** 拟合，然后分别
评估 real inner-val 与每 replicate surrogate inner-val；模型参数同上。targets：

- subject：18-class；metric 为 18 subject recalls 的等权 balanced accuracy；
- length：`W01_04|W05_16|W17_PLUS`，先 per-subject balanced accuracy 再 18-subject macro；
- power：`P_LOW|P_HIGH`，排除4 edge-unavailable rows，先 per-subject balanced accuracy再 macro；
- session：冻结数据只有 session=1，不拟合伪分类器；要求 real/surrogate exact constant parity。

每 replicate、每 estimable probe 要求
`abs(metric_surrogate-metric_real)<=0.05`。probe 只检查 nuisance transfer，不允许作为 model
feature、semantic outcome 或 family selection optimization。

### 28.6 amplitude、endpoint、mask 与 coverage tolerances

在 700 个 inner-val eligible rows、三个 replicates 上预注册：

```text
row_amplitude_KS      = max over 105 channels of two-sample ECDF KS(real, surrogate)
row_quantile_shift    = max over channel,q of |q_sur-q_real| / max(channel_RMS_real,1e-12)
gamma_jump/gamma_slip = §27.6 exact definitions
```

先在每 subject 内取 row median。N2 PASS 同时要求，对所有 subject×replicate：

```text
median(row_amplitude_KS)      <= 0.15
median(row_quantile_shift)    <= 0.25
0.5 <= median(gamma_jump_sur)/max(median(gamma_jump_real),1e-12) <= 2.0
0.5 <= median(gamma_slip_sur)/max(median(gamma_slip_real),1e-12) <= 2.0
```

这些是大幅 artifact sanity bounds；real/N2 classifier upper-AUC 仍是联合可识别性的 primary
判据。不得在看到结果后改用 AAFT/IAAFT、window、circular shift、endpoint repair、amplitude
remap、删 subject/row、换 replicate/C/features 或只报有利 aggregate。

mask/window acceptance：valid samples、A1 window count、true-prefix mask 对 real/surrogate exact
一致；surrogate 无 padding；44 short 继续 `FORCED_L0_NO_FRONTEND` 且 no-read。所有 18 subjects
必须在 inner-val classifier 与三个 estimable nuisance probes有合法 metric，否则 N2
`NOT_ADMITTED_INCONCLUSIVE`。

### 28.7 N1 独立裁决与 Gate mechanical outcome

N1 不新增 EEG read。只从 frozen run-017/018 artifacts 重建并复核 199 mappings、block-scope
bijection、unique hashes、fixed points、selection-aware callback API 与 coverage facts。精确状态：

```text
N1 structural integrity       PASS
N1 mechanism/robustness       ADMITTED
N1 primary/fallback           INELIGIBLE_DEGRADED_COVERAGE
N1 new real EEG reads         0
```

不得以 Gate R0 修改 bin、edge、block、coverage denominator、fixed points 或复活 N1 primary。

N2 mechanical decision：

- **PASS_N2_PRIMARY_N1_MECHANISM_ONLY**：§§28.2–28.6 每个 coverage/numerical/A1/classifier/
  nuisance/amplitude/endpoint/mask check 全 PASS；
- **FAIL_NO_PRIMARY_REFERENCE**：任一 scientific threshold 明确 FAIL，或 completed audit 为
  `NOT_ADMITTED_INCONCLUSIVE`；N2 删除 primary，N1 仍只 mechanism，因此 RC-HSG reference-
  calibrated route 按 §20.9 转 ordinary hierarchical selective generation；
- **TECHNICAL_ABORT_NO_OUTCOME**：fixed input/scope/read-count/model convergence/output/validator
  等执行失败；不得把技术失败写成科学 FAIL，不 commit 部分 Gate outcome，保持 Gate R0 未决。

禁止把低 AUC 解释成 real/N2 exchangeable、exact null 或 brain mechanism；它只是未被本冻结
artifact probe 检出的必要 admissibility evidence。

### 28.8 implementation、outputs 与隐私边界

新增：

```text
scripts/audit_gate_r0.py
tests/test_audit_gate_r0.py
artifacts/governance/run019_postcommit_correction.yaml
artifacts/gates/gate_r0_n2_coverage_v1.jsonl
artifacts/gates/gate_r0.yaml
reports/gate_r0.md
runs/2026-08-24_020_gate_r0_reference_integrity.md
```
...
production CLI 只允许 `--output-root --verification-root-a --verification-root-b`，dataset root
必须等于 frozen reader 的 `/home/song/projects/trust_align/01_data_protocol/datasets/zuco_2.0`；
禁止 role/subject/row/replicate/feature/classifier/C/threshold/resume/cache/seed/hash override。
单次 scan 后全部 canonical bytes 在内存 render，写 verification A/B 与 canonical、byte-compare、
atomic replace。

coverage ledger 只允许 committed metadata、replicate IDs、phase seed SHA、read-count、finite/
shape/mask/numerical-pass booleans与 panel flag；不得写 raw/surrogate value、token、feature、FFT、
phase、per-row diagnostic/classifier probability。Gate YAML/report 只保存 aggregate/subject-level
metrics、thresholds、counts、hashes与 decision，floats 用 `float.hex()`；不得保存 classifier/probe
coefficients或 pickle/joblib。真实 EEG-derived artifact 总大小上限 2 MiB，禁止 binary。

### 28.9 tests、safety audit 与服务器验证

tests 至少覆盖：fixed hashes/counts；safe audited reader；exact 3,497 allowlist/once-only reads；
44 short/cal/test no-read；row order；216-cell panel；replicates 1/2/199；seed/replay；R metrics；
A1 identical path/window/mask；2,429 feature order；train-fit-only standardization/model；inner-val-only
Gate；paired balance/no row overlap；18-subject AUC/bootstrap hash/order statistic；subject/length/power/
session probes；0.65/0.05/0.15/0.25/factor-2 boundary mutations；PASS/FAIL/INCONCLUSIVE/technical
abort branches；N1 exact replay no-read；no value/model persistence；triple render/atomic/symlink/path
escape/unknown CLI；AST 禁止 text/outcome/test/candidate/score/p-value/training/full encoder。

服务器至少实际运行：

```text
.venv/bin/python -m unittest discover -s tests -p 'test_audit_gate_r0.py'
.venv/bin/python -m unittest discover -s tests -p 'test_n2_common_phase.py'
.venv/bin/python -m unittest discover -s tests -p 'test_build_n2_sampler_contract.py'
.venv/bin/python -m unittest discover -s tests -p 'test_n1_joint_permutation.py'
.venv/bin/python -m unittest discover -s tests -p 'test_build_n1_sampler_contract.py'
...
全部实际 PASS/no skip，记录数值与 elapsed time/peak RSS。production Gate CLI 只执行一次；真实
eligible reads 必须恰 3,497，short/cal/test/text/outcome/test-identity reads 0。diff 不含 raw/
surrogate/token/feature/probability/model、text/outcome/test、cache/large binary；旧 SPEC/review/run/
sampler/builder/contracts/A1/N1/split artifacts byte-immutable。

### 28.10 状态迁移与两个科学分支

导入新增 `SPEC_V29_REVIEW=DONE`；只有 completed PASS/FAIL audit 才置 `GATE_R0=DONE`。两种
科学 outcome 的共同状态：

```text
task counts                 76 total / 45 DONE / 8 SKIPPED / 22 BLOCKED / 1 READY
last_completed_task         GATE_R0
recommended_next_task       S0_SEMANTIC_ITEM
S0_SEMANTIC_ITEM            READY; owner=CHATGPT_OR_AUTHOR
last_run                    runs/2026-08-24_020_gate_r0_reference_integrity.md
route.locked                null
test_status                 LOCKED_UNTIL_ROUTE_LOCK
all later Gate outcomes     null
project.spec_version        v2.9
project.spec_path           guide/RC_HSG_Paper_Spec_v2_9_2026-08-24.md
project.baseline_commit     4fa6fadc8bdee0d163acc8bf9ee48aeac4d3095d
project.reviewed_commit     4fa6fadc8bdee0d163acc8bf9ee48aeac4d3095d
```

PASS 分支：

```text
gates.gate_r0.outcome       PASS_N2_PRIMARY_N1_MECHANISM_ONLY
route.primary               RC-HSG
N2 primary                  ADMITTED
N1                          MECHANISM_ROBUSTNESS_ONLY
B_V4                        CLOSED_BY_GATE_R0_N2_PASS
repository_status           RC_HSG_V29_GATE_R0_N2_PRIMARY_ADMITTED_SEMANTIC_SCHEMA_REVIEW_PENDING
```

FAIL/INCONCLUSIVE 分支：

```text
gates.gate_r0.outcome       FAIL_NO_PRIMARY_REFERENCE
route.primary               ORDINARY_HIERARCHICAL_SELECTIVE_GENERATION
N2 primary                  NOT_ADMITTED
N1                          MECHANISM_ROBUSTNESS_ONLY
B_V4                        CLOSED_BY_GATE_R0_NO_PRIMARY_REFERENCE
repository_status           RC_HSG_V29_GATE_R0_FAILED_ORDINARY_HSG_SCHEMA_REVIEW_PENDING
```

FAIL 后 reference-dependent tasks 暂不由 Codex 自行 SKIP/重写；它们保持 BLOCKED，等待下一版
author route review 随 `S0_SEMANTIC_ITEM` 精确迁移。两分支都把 B_V3 resolver
`S0_SEMANTIC_ITEM` 从 blocked sentence 移出并设 sole READY；B_V3/B_V5/B_V6 其余内容保持。
同步 AGENTS、AI_START_HERE、HANDOFF、CODEX_NEXT_TASK、TASKS、PROJECT_STATE、implementation
matrix、snapshots、validator/status/tests。

### 28.11 run-020 硬停止线与 prime 目标边界

completed Gate 后 commit/push `main`，fetch，要求 `HEAD=origin/main`、clean、post-push
validator/status PASS，然后 STOP。不得实现 `S0_SEMANTIC_ITEM`、读取 stimulus/semantic outcome、
打开 cal/test、生成 candidate/reference score、训练 A/F/GLM、执行 calibration、method leakage、
route lock、Gate R/C/H、Mechanism A 或 main experiment。

Gate R0 PASS 只提高“reference family 可进入方法”的可行性，不是 prime 目标成功。论文的
**性能提升**仍须以后 Gate R 证明 RC-HSG 在同一 risk 下优于 Absolute-HSG；**层级机制**须
Gate H/ablation 证明 hierarchy/reference feature 的独立 utility；**EEG semantic separation**
只由 non-blocking Mechanism A 支持，不能升级成生物因果机制。下一轮 ChatGPT/author 必须先
冻结 L1–L3 typed semantic targets、deterministic projections/evaluator、L4 no-new-unit renderer
与 candidate firewall；在这些完成前不得估计 performance 或 mechanism outcome。
