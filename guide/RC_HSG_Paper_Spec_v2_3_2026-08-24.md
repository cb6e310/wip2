# RC-HSG 小论文统一规格（真实 A 前端自检版 v2.3，2026-08-24）
...
本文件继承 v1.2–v2.2 的算法审查、风险纪律、仓库治理、输入准入、stimulus
identity/grouping、run-011 joint-split/population 证据、run-012 RC-HSG activation 与
run-013 synthetic A-interface evidence。
它是论文、实验和实现的共同合同，不是结果报告。§12–§19 是不可改写的历史/物理证据；
§20 是 RC-HSG 科学重构；§21 是 synthetic A-interface 与短片段路由；§22 是当前真实
frontend 自检、leakage-audit 分层和下一动作的权威增量。Codex 只负责实现已冻结决定，
不得重新研究 reference priority、feature/model family、split、Gate、backbone、短片段
处理或真实数据审计抽样策略。

> **版本权威**：v2.3 是下一次 Codex 导入后应激活的 SPEC。导入前远程
> `main@237788090dcb20e533f304f63ae8feb2f545fe0b` 已完成 run 013，当前恰有
> 68 tasks、31 DONE、8 SKIPPED、28 BLOCKED，唯一 READY/推荐任务为 owner=`CODEX` 的
> `S0_A1_FRONTEND`。下一 run 只实现 §22 冻结的 107-row outer-train real-frontend
> self-check、验证 44-row short bypass、拆分 leakage task graph，并把早期
> `S0_LEAKAGE_AUDIT` 置为唯一 READY 后停止；不得全量准入、训练、读取 cal/test EEG
> 或任何 outcome、执行后期 method leakage audit 或任何 Gate。
